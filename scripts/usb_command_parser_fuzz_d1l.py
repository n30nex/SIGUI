#!/usr/bin/env python3
"""Build and execute the bounded production USB command parser fuzz gate."""
...
MANIFEST_PATH = ROOT / "tests" / "usb_command_parser" / "manifest.json"
CORPUS_PATH = ROOT / "tests" / "usb_command_parser" / "corpus.json"
DEFAULT_RUNS = 100_000
DEFAULT_SEED = 13_746_277
NATIVE_MARKER = "native USB command parser: ok (100000 deterministic cases)"
...
def run(command: Sequence[str], *, timeout: int, env: dict[str, str]) -> subprocess.CompletedProcess[str]:
...
def load_inputs() -> tuple[dict, list[tuple[str, bytes, str]], dict[str, str]]:
    ...
    if (
        ...
        or manifest.get("target") != "usb_protocol_command_parser"
        ...
        or manifest.get("max_input_bytes") != 320
        ...
        raise GateFailure("USB parser manifest policy drifted")
    ...
    if corpus_pin != {
        ...
        raise GateFailure("USB parser corpus pin drifted")
    ...
    for seed in corpus.get("seeds", []):
        ...
        if not isinstance(name, str) or re.fullmatch(r"[a-z0-9_]+", name) is None:
            raise GateFailure("USB parser corpus seed name is invalid")
        payload = bytes.fromhex(seed.get("hex", ""))
        digest = hashlib.sha256(payload).hexdigest()
        if not payload or len(payload) > 320 or digest != seed.get("sha256"):
            raise GateFailure(f"USB parser corpus seed failed validation: {name}")
        seeds.append((name, payload, digest))
    if len(seeds) != 7 or len({item[0] for item in seeds}) != 7:
        raise GateFailure("USB parser corpus seed count or names drifted")

    source_pins = manifest.get("source_pins")
    if not isinstance(source_pins, dict) or not source_pins:
        raise GateFailure("USB parser source pins are missing")
...
def execute(args: argparse.Namespace) -> tuple[int, dict]:
    ...
    report: dict = {
        "schema_version": 1,
        "artifact_type": "d1l_usb_command_parser_fuzz",
    ...
    try:
        ...
        report["manifest"] = {
            "path": "tests/usb_command_parser/manifest.json",
        ...
        with tempfile.TemporaryDirectory(prefix="d1l-usb-command-fuzz-") as temporary:
            build = Path(temporary)
            native = build / "usb_command_parser_test"
            fuzz = build / "usb_command_parser_fuzz"
            common = [
            ...
            if native_completed.stdout.strip() != NATIVE_MARKER:
                raise GateFailure("native USB parser marker drifted")
            ...
            fuzz_compile = [
                *common, "-fsanitize=fuzzer,address,undefined",
                str(ROOT / "main/comms/usb_command_parser.c"),
            ...
            fuzz_command = [
                ...
                "-max_len=320", "-print_final_stats=1",
