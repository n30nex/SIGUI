...
- `DESKOS_SD_MOUNT` is the deliberate SD-touch request used by `storage mount`.
  ...
  report `state=mount_pending`, and tries the Arduino/SdFat filesystem mount
  ...
  1 MHz SD clock and retries once after resetting the SD wrapper state. No electrical card
