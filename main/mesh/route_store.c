...
#define D1L_ROUTE_STORE_NAMESPACE "d1l_routes"
...
{
    nvs_handle_t handle;
    esp_err_t ret = nvs_open(D1L_ROUTE_STORE_NAMESPACE, NVS_READWRITE, &handle);
    if (ret != ESP_OK) {
        return ret;
    }

    fill_blob(&s_blob_scratch);
    ret = nvs_set_blob(handle, D1L_ROUTE_STORE_KEY, &s_blob_scratch, sizeof(s_blob_scratch));
    if (ret == ESP_OK) {
...
esp_err_t d1l_route_store_init(void)
{
    ...
    ret = nvs_get_blob(handle, D1L_ROUTE_STORE_KEY, &s_blob_scratch, &len);
    if (ret == ESP_ERR_NVS_NOT_FOUND) {
        ...
        clear_ram();
        ret = nvs_erase_key(handle, D1L_ROUTE_STORE_KEY);
        if (ret == ESP_ERR_NVS_NOT_FOUND) {
            ret = ESP_OK;
        }
        if (ret == ESP_OK) {
