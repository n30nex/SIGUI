...
Current public-release status: **not ready to tag**. Live and last strict-verified `main` is `c88b341abdddb87913ca6b0443804547acadca16` through PR #168. Exact-main Actions `29515366961` passed 1,196 host plus 32 checksum tests, 1,008 wire vectors, 931 oracle checks, 100,000 wire plus 100,000 advert fuzz executions with zero findings, and five artifacts / 46 entries across 339 files; receipt SHA-256 is `254ca1982f343c74d8d78c3afd6e42f9cfa02a0447d1bb8e2afdf8353e34c1eb`. PR #167 banks explicit fail-closed legacy protocol-timestamp migration. PR #168 closes the known P2 Admin direct-response re-derivation/session-invalidation gap and banks contact-targeted TRACE for one unique retained contact with a current-boot nonexpired `DIRECT_PROVEN` one-byte path. Multi-byte/multi-hop TRACE, on-device TRACE UI, room administration, mutations, controlled-peer RF, physical UI/time/RF/hardware acceptance, WP-04/WP-10/WP-12/WP-18, and all release gates remain open. Reporting remains 80% capability implementation, 74% weighted release progress, and 0 of 11 final gates green. Use `include_sd_bridge=false` unless SD/RP2040 evidence changed. No tag may be cut until the exact release-commit gate is green.
...
| Nodes, contacts, routes | Partial | Heard nodes, contacts, route trace/detail, role browser, and diagnostics exist. PR #165 strict-banks exact, capacity-bounded Chat/Repeater/Room/Sensor/Unknown totals from the same rendered rows, with noncanonical roles failing closed to Unknown and no inferred role counts. PR #123 banks the reliable-path software boundary, but complete Node detail/capability actions, signed Map pins, real trace/network tools, max-record live updates, controlled-peer route/TRACE, exact-candidate physical Nodes/Map UI, dependencies, and the required WP-16 artifact remain open. |
| Repeater/room administration | Partial | PR #166 strict-banks bounded production read-only repeater login/status behavior with exact contact/identity revalidation, bounded deadlines, volatile replay protection, and secret zeroization. Raw Admin TX is service-queued, but Admin runtime/session parsing and mutations remain caller-side, so WP-06 sole-owner completion is open. PR #168 closes the direct-response derivation/session-invalidation P2 on exact main. Room administration, mutations, truthful Admin UI, controlled-peer RF, exact-candidate hardware acceptance, and WP-18 closure remain open. |
