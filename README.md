...
Current public-release status: **not ready to tag**. The fast release path is issue-sized: start from current `main`, change one P0, run focused host tests, run the full host suite, use GitHub Actions with `include_sd_bridge=false` unless SD/RP2040 evidence changed, and run only the hardware proof named by the selected issue. PR #56 proves the current five-destination hierarchy from source `51258ba` / Actions `29060900359`: the COM12 Home capture matched firmware/host CRC `E72745BA`, passed the simulator diff, and a three-round all-tab health probe finished with zero failures, empty crashlog, `public_rf_tx=false`, and `formats_sd=false`. PR #51, PR #40, and PR #33 remain useful history for superseded Home layouts. PR #35 closed the issue #2 keyboard gate on COM12 from Actions run `28727064923` with `ui_compose_keyboard_capture_d1l.py --targets all`, `capture_count=12`, `public_rf_tx=false`, and `formats_sd=false`. Core SD card support remains proven on the FAT32 bench artifact set from `1a29876` / run `28714355561`. Release remains blocked by full RF/DM acceptance, the remaining physical SD matrix, manual physical photos/review, 12-hour soak, and the remaining #16/#6 UI modular ownership split after the current navigation/exclusive-modal/chrome/Home/More renderer/keyboard/explicit-root screen boundaries. No release tag should be cut until the release-commit gate is green.
...
| Nodes, contacts, routes | Partial | Heard nodes, contacts, route trace/detail, role browser, and diagnostics exist. The host contact hierarchy keeps Contact Detail to Message plus Contact Options, returns Route/Export/Rename to Contact Options, and requires explicit confirmation before removal. Exact Actions-built COM12 pixels/touch return proof, physical review, and final RF route proof remain open. |
