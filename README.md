...
| Touch Home and shell navigation | Partial | Home uses a full-screen icon launcher with a small title only, colored Time/Wi-Fi/BLE/SD status icons, and no bottom dock; Messages, Nodes, Map, Packets, and Settings keep the bottom dock. PR #51 / Actions `29055441069` proves the tightened Home pixels on COM12, and `ui_home.c` owns the Home renderer and stable enum action boundary. The remaining top-level screen owners, modal exclusivity, deeper single-root invariants, and manual physical photos/review are still open. |
