...
Current public-release status: **not ready to tag**. The fast release path is issue-sized: start from current `main`, change one P0, run focused host tests, run the full host suite, use GitHub Actions with `include_sd_bridge=false` unless SD/RP2040 evidence changed, and run only the hardware proof named by the selected issue. PR #61 / source `4d9f384` passed push Actions `29081187314` and PR Actions `29081214738`, then proved Storage, Card status, and Data locations on COM12 with matching framebuffer CRCs `9878ADFB`, `77726394`, and `42619B0E`, passing simulator diffs, restored scrolling, and a clean three-round probe with `public_rf_tx=false` and `formats_sd=false`. The current Map hierarchy is host-only until its exact Actions-built firmware is captured on COM12. Release remains blocked by full RF/DM acceptance, the remaining physical SD matrix, manual physical photos/review, 12-hour soak, and the remaining #16/#6 UI modular ownership split. No release tag should be cut until the release-commit gate is green.
