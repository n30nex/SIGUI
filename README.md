...
| Touch Home and shell navigation | Partial | Home uses a full-screen icon launcher with a small title only, colored Time/Wi-Fi/BLE/SD status icons, and no bottom dock; Messages, Nodes, Map, Packets, and Settings keep the bottom dock. PR #51 / Actions `29055441069` proves the tightened Home pixels on COM12, `ui_home.c` owns the Home renderer, `ui_settings.c` owns the top-level Settings dashboard and stable actions, and every top-level renderer receives the canonical content root explicitly from `ui_screen.c`. Messages, Nodes, Map, and Packets still need owned renderers; Settings sheets also remain in the shell pending their domain slices. Modal exclusivity, UI-task command ownership, callback-lifetime contracts, and manual physical photos/review are still open. |
