...
Current public-release status: **not ready to tag**. The fast release path is issue-sized: start from current `main`, change one P0, run focused host tests, run the full host suite, use GitHub Actions with `include_sd_bridge=false` unless SD/RP2040 evidence changed, and run only the hardware proof named by the selected issue. PR #56 proves the five-destination Home hierarchy from source `51258ba` / Actions `29060900359`. Merged PR #59, source `d24552e` / Actions `29064260772`, then proved the simplified Contact Detail, Contact Options, and confirmation-only Forget pages on COM12 with exact framebuffer CRC matches, passing simulator diffs, a clean three-round all-tab probe, `public_rf_tx=false`, and `formats_sd=false`. That evidence does not cover the newer Mesh Roles hierarchy, which remains host-complete but pending an exact Actions-built COM12 capture and physical touch review. PR #35 closed the issue #2 keyboard gate on COM12 from Actions run `28727064923`; core SD support remains proven by the FAT32 bench set from `1a29876` / run `28714355561`. Release remains blocked by full RF/DM acceptance, the remaining physical SD matrix, manual physical photos/review, 12-hour soak, and the remaining #16/#6 UI modular ownership split. No release tag should be cut until the release-commit gate is green.
...
| Nodes, contacts, routes | Partial | Heard nodes, contacts, route trace/detail, role browser, and diagnostics exist. Merged PR #59 / `d24552e` / Actions `29064260772` proves the simplified contact hierarchy on COM12 without invoking removal or Public RF. Mesh Roles now opens a simple Rooms/Repeaters menu whose bounded read-only subpages return to that menu; exact Actions-built COM12 Mesh Roles pixels, physical touch review, and final RF route proof remain open. |
