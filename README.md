...
Current public-release status: **not ready to tag**. The fast release path is issue-sized: start from current `main`, change one P0, run focused host tests, run the full host suite, use GitHub Actions with `include_sd_bridge=false` unless SD/RP2040 evidence changed, and run only the hardware proof named by the selected issue. PR #40 proved the prior Home icon layout from commit `b2ca00a` / Actions run `28731948363` with `ui_pixel_capture_b2ca00a_actions_28731948363_COM12.json`, CRC `0DB2C27A`, `public_rf_tx=false`, and `formats_sd=false`; current Home layout changes need current-commit Home proof before release-gate use. The previous COM12 Home pixel proof from PR #33 (`c6a88e2` / PR Actions `28725692751`, merged as `e086312`) remains useful history for the older layout. The broader COM12 UI stability set from Actions-built commit `59610ab` / run `28723265336` still covers targeted UI corruption and scroll probing. PR #35 closed the issue #2 keyboard gate on COM12 from Actions run `28727064923` with `ui_compose_keyboard_capture_d1l.py --targets all`, `capture_count=12`, `public_rf_tx=false`, and `formats_sd=false`. Core SD card support remains proven on the FAT32 bench artifact set from `1a29876` / run `28714355561`. Release remains blocked by full RF/DM acceptance, the remaining physical SD matrix, manual physical photos/review, 12-hour soak, and the remaining #16/#6 UI modular ownership split after the current navigation/modal/chrome/Home state/geometry/keyboard target/layout/focus helper/screen-render/content-root boundaries. No release tag should be cut until the release-commit gate is green.
...
| Touch Home and shell navigation | Partial | Home uses a full-screen icon launcher with a small title only, colored Time/Wi-Fi/BLE/SD status icons, and no bottom dock; Messages, Nodes, Map, Packets, and Settings keep the bottom dock. The committed simulator screenshot reflects the current tightened Home layout; PR #40 host/simulator, Actions run `28731948363`, and COM12 `ui_pixel_capture_b2ca00a_actions_28731948363_COM12.json` prove the prior Home pixels, so current-commit Home proof and manual physical photos/review are still tracked separately. |
