...
Current public-release status: **not ready to tag**. The active [roadmap](docs/ROADMAP.md) incorporates the complete [KRAB 1.0 release audit](docs/KRABSTHOUGHTS.MD) and keeps work issue-sized and fail-closed. The Map/Wi-Fi branch has green Actions plus exact-COM12 boot-loop recovery, Wi-Fi reconnect/reboot, memory, SD, and short-stability proof; physical Map fetch/render/cache acceptance is still open. The standalone ESP-IDF v5.5.4 migration has an exact Actions-generated dependency lock and green firmware/package/checksum jobs, but it is not the production baseline until this combined candidate passes Actions and exact-COM12 SDK/Wi-Fi/radio/SD/Map/reboot proof. Use `include_sd_bridge=false` unless SD/RP2040 evidence changed. Full MeshCore conformance, DM/ACK/retry/PATH/trace, contact/channel parity, runtime durability, remaining SD/RF/physical review, and final soaks also remain release blockers. No tag may be cut until the exact release-commit gate is green.
...
| Touch Home and shell navigation | Partial | Home is a quiet dashboard with Messages, Network, Map, and More destinations plus one Device status card. Non-Home pages use the matching five-item dock; Packets, diagnostics, connections, storage, and device settings live under More. Focused modules own Home, More, and the current Map hierarchy; shared screen/modal boundaries preserve one root and one active nested page. PR #56 proves Home and PR #61 proves Storage. The Map split is host/Actions-tested but still needs current combined-COM12 pixels and physical fetch/render/cache proof. Messages, Network/routes, Packets, remaining settings domains, UI-task command ownership, deeper callback-lifetime contracts, and manual physical review remain open. |
...
| Map and tile cache | Partial | Map uses the built-in OpenStreetMap Standard tile source with no provider editor. The simple path is `Map -> Map options -> Set location or Cache status`; connect Wi-Fi, then open the actual Map. Only its visible current-view 3x3 at one zoom may be requested, cached tiles are reused, and `(c) OpenStreetMap contributors` stays visible. There is no background or area download. Network-suppressed probes never request tiles or mutate Wi-Fi/storage. Map-specific combined-COM12 pixels and live request/render/cancel/cache/heap proof remain pending. |
| Wi-Fi | Partial | Setup UI and bounded serial controls exist and remain disabled by default. Standalone `de79c9f` is hardware-proven for enable/connect/reconnect, saved-profile reboot, bounded redacted scan, and internal/DMA memory floor on ESP-IDF 5.1.7; repeat qualification is required on the combined v5.5.4 artifact. |
