...
Current public-release status: **not ready to tag**. The fast release path is issue-sized: start from current `main`, change one P0, run focused host tests, run the full host suite, use GitHub Actions with `include_sd_bridge=false` unless SD/RP2040 evidence changed, and run only the hardware proof named by the selected issue. PR #37 updates the Home layout and is host/Actions-proven; the previous COM12 Home pixel proof from PR #33 (`c6a88e2` / PR Actions `28725692751`, merged as `e086312`) remains useful history but no longer proves the newest Home pixels. The COM12 compose/input proof from PR #35 (`fce5d82` / Actions `28727064923`, merged as `77c423c`) covers all release-blocking keyboard callers with `public_rf_tx=false` and `formats_sd=false`. Core SD card support remains proven on the FAT32 bench artifact set from `1a29876` / run `28714355561`. Release remains blocked by guided SD install validation, full RF/DM acceptance, the remaining physical SD matrix, manual physical photos/review, 12-hour soak, and fresh COM12 pixel/manual evidence for the newest Home layout once hardware is available. No release tag should be cut until the release-commit gate is green.
...
| Touch Home and shell navigation | Host/Actions-proven | Home uses a full-screen icon launcher with a small title only, colored Time/Wi-Fi/BLE/SD status icons, and no bottom dock; Messages, Nodes, Map, Packets, and Settings keep the bottom dock. PR #37 host/simulator and Actions checks cover the newest layout; COM12 pixel/manual proof still needs to be recaptured for this newest Home screen. |
