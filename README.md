...
Current public-release status: **not ready to tag**. The fast release path is issue-sized: start from current `main`, change one P0, run focused host tests, run the full host suite, use GitHub Actions with `include_sd_bridge=false` unless SD/RP2040 evidence changed, and run only the hardware proof named by the selected issue. PR #51 proved the tightened Home layout from commit `485744e` / Actions run `29055441069` with `ui_pixel_capture_485744e_actions_29055441069_COM12.json`, matching firmware/host CRC `BB15A654`, simulator diff success, `public_rf_tx=false`, and `formats_sd=false`. PR #40 and PR #33 remain useful history for older Home layouts. The broader COM12 UI stability set from Actions-built commit `59610ab` / run `28723265336` still covers targeted UI corruption and scroll probing. PR #35 closed the issue #2 keyboard gate on COM12 from Actions run `28727064923` with `ui_compose_keyboard_capture_d1l.py --targets all`, `capture_count=12`, `public_rf_tx=false`, and `formats_sd=false`. Core SD card support remains proven on the FAT32 bench artifact set from `1a29876` / run `28714355561`. Release remains blocked by full RF/DM acceptance, the remaining physical SD matrix, manual physical photos/review, 12-hour soak, and the remaining #16/#6 UI modular ownership split after the current navigation/exclusive-modal/chrome/Home/Settings renderer/keyboard/explicit-root screen boundaries. No release tag should be cut until the release-commit gate is green.
...
| Touch Home and shell navigation | Partial | Home uses a full-screen icon launcher with a small title only, colored Time/Wi-Fi/BLE/SD status icons, and no bottom dock; Messages, Nodes, Map, Packets, and Settings keep the bottom dock. PR #51 / Actions `29055441069` proves the tightened Home pixels on COM12, `ui_home.c` owns the Home renderer, `ui_settings.c` owns the top-level Settings dashboard and stable actions, every top-level renderer receives the canonical content root explicitly from `ui_screen.c`, and `ui_modal.c` enforces one active sheet while preserving nested parent-return flows. Messages, Nodes, Map, and Packets still need owned renderers; Settings sheets also remain in the shell pending their domain slices. UI-task command ownership, callback-lifetime contracts, and manual physical photos/review are still open. |
