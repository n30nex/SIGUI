...
- Map opens the actual current view with one Options action. The simple hierarchy is `Map -> Map options -> Set location or Cache status` (canonical probes: `map -> map_options -> map_location or map_cache`). Back from either child returns to Map options; Save location returns to the actual Map. Connect Wi-Fi there to load the visible current-view 3x3 at one zoom; cached tiles are reused. There is no area download and no background fetch; `(c) OpenStreetMap contributors` stays visible.
...
- Wi-Fi is off by default but can be explicitly enabled for local setup, bounded `wifi scan`, saved-profile `wifi connect`, and the touch Wi-Fi Setup sheet. Map has a built-in OpenStreetMap Standard source and no source/account/key editor. Network access is limited to a user-visible current-view 3x3 at one zoom while the actual Map is active; cached revisits do not widen that scope. BLE companion runtime, OTA, and hardware proof remain pending.
- The simplified Map hierarchy is host/Actions-tested and present in the standalone Map/Wi-Fi hardware image, but combined ESP-IDF v5.5.4 Map-specific pixels and the physical request/render/cancel/cache flow remain release gates. Its page-open probes are read-only and network-suppressed: no map tile request, no Wi-Fi mutation, no RF, and no storage mutation.
