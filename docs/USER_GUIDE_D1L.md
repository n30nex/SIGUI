...
- The simplified Map hierarchy is host-tested only until exact Actions-built COM12 pixels pass. Its page-open probes are read-only and network-suppressed: no map tile request, no Wi-Fi mutation, no RF, and no storage mutation.
