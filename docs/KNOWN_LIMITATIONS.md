...
- The Phase 3/4 touch shell is implemented with Home, Public `test` action, first Public free-text composer, persisted recent Public rows, Nodes count view, Packet log, Settings, advert sheet, toasts, and lock overlay, but full Public scrollback/search, DMs, persistent nodes, contacts, routes, packet stores, BLE companion, Wi-Fi management, OTA, and contact/route persistence are not implemented yet.
