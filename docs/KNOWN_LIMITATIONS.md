...
- The Phase 3/4 touch shell is implemented with Home, Public `test` action, first Public free-text composer, first contact-row DM composer, first contact detail sheet with DM/favorite/mute actions, first bounded DM thread/detail sheet with `Reply`, persisted recent Public/DM preview rows, newest contact/heard-node rows, route count, Packet log, Settings, advert sheet, toasts, and lock overlay, but full Public scrollback/search, richer full-thread DM history, richer contact editing, route detail views, packet stores, BLE companion, Wi-Fi management, and OTA are not implemented yet.
- The Public composer, contact-row DM composer, contact detail sheet, and bounded DM thread sheet have passed host contract tests, local ESP-IDF build, flash, and serial smoke/backend probes; manual physical touch entry on the composer keyboard and thread/reply/contact sheets is still pending.
