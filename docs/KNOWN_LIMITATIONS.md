...
- The Phase 3/4 touch shell is implemented with Home, Public `test` action, first Public free-text composer, persisted recent Public/DM preview rows, newest contact/heard-node rows, route count, Packet log, Settings, advert sheet, toasts, and lock overlay, but full Public scrollback/search, DM compose/thread workflow, contact detail/editing, route detail views, packet stores, BLE companion, Wi-Fi management, and OTA are not implemented yet.
