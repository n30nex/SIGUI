...
- Map is the actual current-view surface, not a setup dashboard. It has one `Options` action, an unobtrusive zoom/status badge, and the always-visible ASCII attribution `(c) OpenStreetMap contributors`.
...
- Interactive Map network policy is fail-closed: only while the actual Map is visible may firmware request at most the visible current-view 3x3 at one zoom. It must reuse cached tiles and stop when Map is hidden. There is no area download and no background fetch.
