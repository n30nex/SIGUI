...
- After the RP2040 bridge is flashed, run preflight first so it captures `rp2040 ping`, safe `storage status`, explicit `storage mount`, optional `storage diag`, and `health`. The preflight may poll safe status if a build reports `state="mount_pending"`, but the current filesystem mount path runs on the protocol-handling core because Actions run `28606586139` / COM16 evidence showed the Arduino `SD`/`SDFS` path can wedge when invoked from the RP2040 core1 worker. Use `storage filecanary` or `python .\scripts\sd_file_canary_d1l.py --port COM12` only once the ready file gate is available, then `python .\scripts\sd_map_tile_canary_d1l.py --port COM12 --token map1` for the map-tile cache proof. These canaries are serial-only and do not send Public RF or format SD.
