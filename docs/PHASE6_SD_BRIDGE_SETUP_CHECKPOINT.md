...
- Current follow-up: the direct high/dedicated SdFat mount did not produce a ready FAT32 filesystem even though the user-confirmed FAT32 card was inserted. The bridge now routes actual filesystem work through the Arduino `SD`/`SDFS` wrapper documented by Seeed, tries filesystem mount on every raw-present high/low, dedicated/shared candidate before declaring the card unmountable, enables SdFat SPI command CRCs with `USE_SD_CRC=1`, disables the RP2040 SdFat array-transfer path with `USE_SPI_ARRAY_TRANSFER=0`, and force-cycles the selected SD rail before card init. The latest COM12/COM16 proof moved from `mount_error=23` / `mount_data=8` to `mount_error=1` / `mount_data=0`, then to a stricter raw probe result of `probe_error=3` / `probe_data=0` when false `CMD0=0x00` ready-state responses were rejected. Use `mount_error`/`mount_data` as firmware-side diagnostics; do not treat reformatting the card as the fix.
