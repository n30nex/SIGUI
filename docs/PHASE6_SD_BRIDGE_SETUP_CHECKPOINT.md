...
- Current follow-up: the direct high/dedicated SdFat mount did not produce a ready FAT32 filesystem even though the user-confirmed FAT32 card was inserted. The bridge now tries filesystem mount on every raw-present high/low, dedicated/shared candidate before declaring the card unmountable, the Actions RP2040 build enables SdFat SPI command CRCs with `USE_SD_CRC=1`, disables the RP2040 SdFat array-transfer path with `USE_SPI_ARRAY_TRANSFER=0`, and each explicit SdFat mount/probe force-cycles the selected SD rail before card init because the latest COM12/COM16 proof moved from `mount_error=23` / `mount_data=8` to `mount_error=1` / `mount_data=0` after CRC was enabled. The raw probe now requires a proper `CMD0` idle response before it reports card presence, so false ready-state responses do not hide the firmware-side mount failure. Use `mount_error`/`mount_data` as firmware-side diagnostics; do not treat reformatting the card as the fix.
