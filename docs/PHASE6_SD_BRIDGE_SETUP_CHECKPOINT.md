...
- Current follow-up: the direct high/dedicated SdFat mount did not produce a ready FAT32 filesystem even though the user-confirmed FAT32 card was inserted. Actions run `28604061282` for commit `4064823b72d96e1e39bc1252b50fa7e39e079505` passed and was flashed locally; COM12 evidence `artifacts/hardware/com12/rp2040_preflight_4064823_actions_28604061282_COM12.json` proved `rp2040 ping` before SD touch but the Arduino `SD`/`SDFS` filesystem path wedged later status/diag replies during `storage mount`. The next bridge artifact probes the high/low dedicated/shared raw SPI matrix before any filesystem mount attempt, then mounts only raw-present candidates, so the FAT32 validation card should produce bounded firmware-side probe diagnostics instead of `ESP_ERR_TIMEOUT`. The latest proof sequence moved from `mount_error=23` / `mount_data=8` to `mount_error=1` / `mount_data=0`, then to a stricter raw probe result of `probe_error=3` / `probe_data=0` when false `CMD0=0x00` ready-state responses were rejected. Use `mount_error`/`mount_data` and `probe_error`/`probe_data` as firmware-side diagnostics; do not treat reformatting the card as the fix.
