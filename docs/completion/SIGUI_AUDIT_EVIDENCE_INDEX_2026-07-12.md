...
| Item | Value |
|---|---|
| Live merged `main` | `d5acf0a80af4ff533f48105ea84844bd0d9af6c3` |
| WP-01 exact source candidate | `092293f2311a24c9899bc9bf343ab014c4ba0411` |
| Active PR stack | no pull request for the current slice; `codex/wp04-trace-discovery` is local implementation only |
| Candidate integration state | PRs #62, #64, #80, and #84-#99 are merged; WP-03 proof is banked, WP-04 production/oracle Ed25519, schema-v5 ACK durability, learned-route selection, and the exact IDF receipt are merged while TRACE/admin/persisted identity/RF/physical closure remain open |
...
| SDK | ESP-IDF 5.5.4 |
| Candidate host suite | 944 passing tests and 32 checksum-contract tests in exact-main Actions `29317376691` |
| Candidate CI | Exact PR #99 and merged-main runs are green; production/oracle/runtime Ed25519, exact IDF receipt, ACK durability, route selection, conformance/fuzz, ESP32 firmware, package, SBOM, provenance, notices, and downloaded checksums pass |
| Conformance closure | false; defined-arithmetic signed-advert integration plus ACK-durability and route-selection software evidence are merged, while canonical TRACE, admin, persisted identity, retained-state, real-peer RF, and physical proof remain |
| Release status | not ready to tag; Actions dry audit receipt `83223d50b78044b497e5db43f4dba00353b535c43a978613318de73c23fdeca8` remains fail-closed with 33 P0 failures / 35 overall |
