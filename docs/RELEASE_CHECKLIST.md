...
- [ ] WP-01 exact-pair qualification remains open on `07322ed`: official FAT32 smoke and basic file/Map/export canaries passed, but asynchronous `storage diag raw` outlived fixed settle after a clean zero-counter `READY_SD` preflight, overlapped the one-second retained worker, and contaminated the retained counters before packet append failed; reboot correctly cancelled with `ESP_ERR_INVALID_STATE`. Isolate raw diagnostics in a maintenance boot, reset/reflash the exact Actions artifacts, require a fresh zero-counter `READY_SD` preflight, and rerun the retained/reboot/inserted/remove-reinsert/soak gate. Do not accept a 750 ms ordinary-timeout increase as closure.
