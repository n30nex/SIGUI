...
- [ ] The next-head controlled-reboot repair uses one shared 15-second deadline in this order: storage-manager quiesce, retained-worker force flush with remaining time, final RP2040 bridge/UART quiesce with remaining time, final deadline check, acknowledgement/flush, then restart while both locks remain held. Any pre-restart failure cancels and releases/resumes acquired components. Check only after the new SHA passes host/Actions and checksum-verified exact-artifact COM12 proof.
