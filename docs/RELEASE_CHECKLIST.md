...
- [x] Current COM12 hardware pixel capture artifact captured from Actions-built `59610ab` / run `28723265336` with `scripts/ui_capture_d1l.py`, 480x480 RGB565 PNG reconstruction, matching CRC `B7D2D890`, passing simulator/reference diff, and `public_rf_tx=false` / `formats_sd=false` proof.
- [x] Current COM12 compose keyboard capture artifact captured from Actions-built `59610ab` / run `28723265336` for Public short/long and DM short/long targets with PNG/RGB565 captures, `public_rf_tx=false`, and `formats_sd=false`.
...
- [ ] Fresh issue-sized evidence still required for the remaining P0s: the specific remaining keyboard/sheet proof named by the issue, full RF/DM acceptance, remaining physical SD release matrix, physical photos/manual UI review, and 12-hour soak. Targeted UI corruption, Home pixel-capture simulator diff, scroll proof, and Public/DM compose-keyboard capture are already proven on `59610ab` / Actions `28723265336`; do not rerun them for unrelated PRs. Collect one artifact set per issue; save the full sweep for final release readiness.
