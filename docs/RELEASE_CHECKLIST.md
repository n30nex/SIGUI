...
- [ ] Build the new Mesh Roles hierarchy in Actions, flash that exact commit on COM12, and capture `ui scroll-probe mesh_roles|mesh_rooms|mesh_repeaters`. Physically verify root Back closes to Packets, child Back returns to Mesh Roles, long lists scroll inside their bounded panel, and tapping an observation row performs no RF or destructive action.
...
- [ ] The replacement full-height Mesh Roles root/Rooms/Repeaters hierarchy is host-tested but still needs exact Actions-built COM12 page captures and physical Back/scroll review.
...
- [ ] Fresh issue-sized evidence still required for the remaining P0s: exact Actions/COM12 Mesh Roles root/Rooms/Repeaters pixels and Back/scroll behavior; #16/#6 owned full-height Messages, Network/routes, Map, Packets, contact, and settings page domains; UI-task ownership; remaining sheet-specific keyboard submit/cancel ownership; deeper single-root/callback invariants; full RF/DM acceptance; remaining physical SD release matrix; physical photos/manual UI review; and 12-hour soak. Merged PR #59 / `d24552e` / Actions `29064260772` already proves the contact hierarchy and quick all-tab health; PR #56 proves Home; `59610ab` / Actions `28723265336` proves the targeted longer corruption/scroll path; and PR #35 / Actions `28727064923` proves all keyboard callers. Do not rerun closed proofs for unrelated pixel-neutral PRs. Collect one artifact set per issue; save the commit-matched full sweep for final release readiness.
