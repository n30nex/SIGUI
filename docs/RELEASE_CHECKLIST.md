...
- [x] Historical controlled MeshCore Public `test` TX/RX was validated with a local bot; current automated RF evidence is DM/`#test` only.
...
- [ ] Repaired exact artifact no longer reproduces the rejected `3aefb9f` `route_persist` stack-overflow PANIC. Require `health.retained_task_stack_free_bytes >= 4096` after late-SD reconciliation, file canary, retained Public/DM/route/packet canary, controlled reboot, and a passive SD-aware soak; crashlog must gain no PANIC entry. A command timeout must abort the retained runner and SD-aware soak without retrying, sending later commands, or rebooting into in-flight console work.
