...
- [ ] Fresh issue-sized evidence still required for the remaining P0s: the specific remaining keyboard/sheet proof named by the issue, full RF/DM acceptance, remaining physical SD release matrix, physical photos/manual UI review, and 12-hour soak. The current Home pixel-capture simulator diff is proven on PR #33 (`c6a88e2` / PR Actions `28725692751`, merged as `e086312`); targeted UI corruption, scroll proof, and Public/DM compose-keyboard capture are already proven on `59610ab` / Actions `28723265336`. Do not rerun them for unrelated PRs. Collect one artifact set per issue; save the full sweep for final release readiness.
