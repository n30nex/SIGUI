...
- [x] Guided public SD install runner added: `scripts/guided_sd_install_d1l.py` can download or accept GitHub Actions artifacts, verify checksums, flash the ESP32 app on COM12, guide the RP2040 COM16 official-smoke and bridge-restore UF2 steps, run post-restore SD checks, refuse forbidden local ports, preserve `formats_sd=false`, and emit one `d1l-guided-sd-install-*.json` report for the release gate.
...
- [x] Current COM12 Home hardware pixel capture artifact captured from PR #33 (`c6a88e2` / PR Actions `28725692751`, merged as `e086312`) with `scripts/ui_capture_d1l.py`, 480x480 RGB565 PNG reconstruction, matching CRC `ED8A8E31`, passing simulator/reference diff, and `public_rf_tx=false` / `formats_sd=false` proof.
...
- [x] Expanded issue #2 compose/input keyboard capture from PR #35 (`fce5d82` / Actions `28727064923`, merged as `77c423c`) using `ui_compose_keyboard_capture_d1l.py --targets all` for Public/DM compose, Public search, Packet search, contact edit, onboarding, map location/provider, and Wi-Fi SSID/password with `public_rf_tx=false` and `formats_sd=false`.
...
- [x] Evidence-based release gate audit script added and uploaded by CI; hardware evidence is metadata-strict for commit-matched UI/scroll/DM/SD/RF/manual/soak artifacts so stale evidence cannot pass a final release audit. The SD gate rejects obsolete format-capable preflight guidance and requires no-device-format evidence (`formats_sd=false`), and `guided_sd_install_validation` now requires a current full guided install report rather than scattered thread context.
...
- [ ] Fresh issue-sized evidence still required for the remaining P0s: guided SD install validation, full RF/DM acceptance, remaining physical SD release matrix, physical photos/manual UI review, and 12-hour soak. Home pixel capture is proven on PR #33 (`c6a88e2` / PR Actions `28725692751`, merged as `e086312`); targeted UI corruption and scroll proof are proven on `59610ab` / Actions `28723265336`; expanded keyboard/sheet proof is proven on PR #35 (`fce5d82` / Actions `28727064923`, merged as `77c423c`). Do not rerun them for unrelated PRs. Collect one artifact set per issue; save the full sweep for final release readiness.
