...
- [ ] Flash the exact Actions-built contact-hierarchy commit on COM12 and use the non-destructive `ui scroll-probe contact_detail|contact_options|contact_forget|contact_route` targets for pixel evidence. The `contact_forget` target may display the confirmation page but must never invoke its red confirmation callback; manual Route/Export/Rename touch-return review remains separate.
