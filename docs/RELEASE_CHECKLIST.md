...
- [ ] Move the production build from EOL ESP-IDF v5.1.x to a supported SDK baseline (target v5.5.x subject to Seeed BSP qualification), or approve a documented maintained security-patch plan; then repeat the complete Actions and hardware gate matrix.
...
- [ ] Build the Map hierarchy in Actions and capture `ui scroll-probe map|map_options|map_location|map_cache` on COM12. The hardware artifact must include successful `map tiles status` rows before and after Map automation with the same `network_requests` count, prove `network_tx=false`, `map_network_requests=false`, no Wi-Fi/RF/storage mutation, and no unsuppressed `ui tab map`; manual physical touch/photos remain open.
