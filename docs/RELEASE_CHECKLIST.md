...
- [x] Evidence-based release gate audit script added and uploaded by CI; hardware evidence is metadata-strict for commit-matched UI/scroll/DM/SD/RF/manual/soak artifacts so stale evidence cannot pass a final release audit. The SD gate rejects obsolete format-capable preflight guidance and requires no-device-format evidence (`formats_sd=false`). Current `1a29876` / Actions `28714355561` local autonomous audit reports `ready_for_public_release=false`, `failed_count=13`, and `p0_failed_count=12` with open P0 evidence gates.
...
- [ ] Fresh current-commit issue-sized evidence still required for the remaining P0s: targeted UI corruption probe, pixel-capture simulator diff, compose-keyboard capture, any specifically named scroll/layout proof, full RF/DM acceptance, remaining physical SD release matrix, physical photos/manual UI review, and 12-hour soak. Collect one artifact set per issue; save the full sweep for final release readiness.
