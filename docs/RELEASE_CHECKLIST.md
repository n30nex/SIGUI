...
- [ ] Issue #63 selected-target policy passes: the firmware job uses the exact version-pinned `espressif/idf:v5.5.4` tag, the committed component lock records IDF 5.5.4, and the release audit rejects missing, moving, EOL, unapproved, or stale selections. These configuration checks alone do not qualify the SDK.
- [ ] ESP-IDF Component Manager generates `dependencies.lock` in the version-pinned Actions environment; the exact generated file and diff are archived, reviewed, and committed without hand-editing its generated hash, and the qualifying repeat Actions build leaves it unchanged.
- [ ] The complete host suite and version-pinned Actions firmware/package/checksum path pass for one commit, with run, resolved image identity when available, dependency lock, and artifact metadata retained together.
- [ ] The matching Actions artifact is flashed to exact COM12; serial `version` reports `"idf":"v5.5.4"`, and board/display/touch/Wi-Fi/RF/RP2040-SD/Map/health/reboot/post-power-cycle smoke passes without weakening safety policy. COM16 is used only for an explicitly required RP2040 proof.
