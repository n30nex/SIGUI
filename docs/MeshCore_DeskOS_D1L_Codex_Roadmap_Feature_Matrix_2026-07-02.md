...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Online research matched Seeed's official D1L/RP2040 MicroSD path and Arduino-Pico's SPI1 `SD.begin` support; Actions run `28622219945` for `8c9e5f9` adds GPIO7 SD-detect evidence and reports the all-zero selected-card path as `state=error note=sd_probe_rejected_card` instead of ordinary `no_card`. Direct COM16 USB shows `detect=low`, `detect_driven=1`, `probe_err=4`, CS-high idle `255`, and MISO high when idle, while selected CMD0/CMD8/R7 bytes remain `0`. This is firmware-side SPI/power/select initialization work, not a card-format task. | Actions-built bridge artifact flashed locally; `artifacts/hardware/com16/rp2040_direct_preflight_8c9e5f9_actions_28622219945_COM16.json` plus `artifacts/hardware/com16/rp2040_direct_diag_poll_8c9e5f9_actions_28622219945_COM16.json` report `ready_for_sd_acceptance=false`, `formats_sd=false`, `public_rf_tx=false`, `detect=low`, accepted SPI pins, CS-high idle `255`, idle MISO high, and selected-card ready/response bytes stuck at `0`, with COM11/COM29 not opened |
