...
| A. FAT32 boot auto-provision | Blocked before FAT parsing | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 validation card still fails before the ready file-operation gate. Online research matched Seeed's official D1L/RP2040 MicroSD wiring/init shape, Arduino-Pico's SPI1 pin/SPI setup, and the SD SPI requirement for CS/MOSI-high clocks before CMD0. Actions-built diagnostics now rule out bridge protocol, filesystem parsing, Seeed `setTX`/`setRX` aliases, inverted CS, safe pairwise SCK/MOSI, CS/MOSI, and SCK/CS swaps, byte-boundary/leading-zero recovery, and the minimal Seeed SD-only smoke firmware. An attempted SPI mode-3 diagnostic (`4b90291`, Actions `28639072595`) wedged the bridge after a pending diag line and was reverted (`5f06eaf`) rather than kept. Branch head was restored from downloaded Actions run `28639309122` and COM16 again answers `DESKOS_SD_PING`, but `DESKOS_SD_MOUNT` still reports `state=error note=sd_probe_rejected_card probe_err=3 probe_data=0 detect=low`. This is firmware-side power/select/reset-entry or board/card electrical-state work, not an SD-format task. | GitHub Actions only: run `28639309122` passed for `5f06eaf`; downloaded RP2040 UF2 SHA-256 `040D90497EE2300B48E82A1F23AC382F83639A1743EDC513A2824CA13ABF42C8` flashed to COM16; `artifacts/hardware/com16/rp2040_restored_ping_mount_5f06eaf_actions_28639309122_COM16.json` reports `formats_sd=false`, `public_rf_tx=false`, `DESKOS_SD_PING ... sd_touch=0`, and `DESKOS_SD_MOUNT state=error note=sd_probe_rejected_card probe_err=3 probe_data=0 detect=low`; COM11/COM29 were not opened |
...
| D. Policy-compliant tile downloader | Serial primitive and provider setup exist; touch live download hidden until tile rendering proof | Added provider-required map state, user-facing no-tile guidance, attribution requirement, hard URL-template guard rejecting public OSM bulk tile hosts, explicit serial `storage map-tile-download` HTTPS streaming into the SD cache, persisted touch provider/attribution/zoom setup, and Settings/Map Tiles entry. Touch live downloads now report `tile_render_pending` instead of exposing a `Download` action until provider, Wi-Fi, SD, location, attribution, and tile rendering proof all pass. | `python -m pytest tests/test_storage_status_contract.py tests/test_connectivity_manager_contract.py tests/test_settings_contract.py tests/test_ui_shell_contract.py tests/test_ui_simulator.py -q`; `python .\tools\ui_simulator.py --out artifacts\ui-sim-phase-d-map-tiles --view map --view map_tiles_sheet --view settings` |
