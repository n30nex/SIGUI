...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Actions run `28606586139` proved the already-powered Seeed-style `SD.begin(13, 1000000, SPI1)` path can wedge when run from the RP2040 core1 worker; the next Actions artifact runs explicit filesystem mount on the protocol-handling core. This is firmware-side, not a card-format task. | Actions-built bridge artifacts flashed locally; `artifacts/hardware/com16/rp2040_direct_probe_151ab6f_actions_28606586139_COM16.json` stops at `state=mount_pending` / `note=filesystem_mounting`, `formats_sd=false`, with no Public RF |
