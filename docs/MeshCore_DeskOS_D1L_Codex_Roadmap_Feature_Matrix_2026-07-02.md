...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Actions run `28609861698` for `ce90215` proved the Arduino-Pico `SPI1.setCS(13)` plus `SD.begin(13, SPI1)` path keeps the bridge responsive, but direct COM16 USB still reports `state=no_card`, `probe_err=4`, and all diagnostic candidates `*_e=4`. The current firmware follow-up explicitly idles CS high, starts SPI1 before the library mount, and adds raw `CMD0`/`CMD8`/R7 diagnostic bytes. This is firmware-side, not a card-format task. | Actions-built bridge artifact flashed locally; `artifacts/hardware/com16/rp2040_direct_preflight_ce90215_actions_28609861698_COM16_retry.json` reports `ready_for_sd_acceptance=false`, `formats_sd=false`, `public_rf_tx=false`, `probe_err=4`, with no ESP32 flash because the only ESP32-S3 port currently enumerated is forbidden `COM11` |
