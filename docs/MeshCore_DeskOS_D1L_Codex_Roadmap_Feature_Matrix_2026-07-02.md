...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Actions run `28619773565` for `0e2d83c` proved the Seeed-style `SD.begin(13, 1000000, SPI1)` build passes Actions and keeps the bridge responsive, but direct COM16 USB still reports `state=no_card` / `probe_err=4`. The completed diagnostic shows GPIO10/11/12/13 pin acceptance, CS-high idle `255`, and MISO high when idle, while the selected card-response path is held low before and during commands (`hd_c0r=0`, `hd_c8r=0`, `hd_c0=0`, `hd_c8=0`, R7 `[0,0,0,0]`). This is firmware-side SPI/power/select initialization work, not a card-format task. | Actions-built bridge artifact flashed locally; `artifacts/hardware/com16/rp2040_direct_preflight_0e2d83c_actions_28619773565_COM16.json` plus `artifacts/hardware/com16/rp2040_direct_diag_poll_0e2d83c_actions_28619773565_COM16.json` report `ready_for_sd_acceptance=false`, `formats_sd=false`, `public_rf_tx=false`, accepted SPI pins, CS-high idle `255`, idle MISO high, and selected-card ready/response bytes stuck at `0`, with COM11/COM29 not opened |
