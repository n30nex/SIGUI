...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails the bridge filesystem mount while the raw SPI probe detects the card. This is firmware-side, not a card-format task. | Actions-built bridge artifacts flashed locally; COM12/COM16 evidence still reports `raw_card_present_mount_failed` / `not_fat32_or_unmountable` with `formats_sd=false` |
