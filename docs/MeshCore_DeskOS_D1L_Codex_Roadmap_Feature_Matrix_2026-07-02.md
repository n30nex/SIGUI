...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Actions run `28605782601` kept the bridge responsive on direct COM16 USB but still rejected the inserted FAT32 card at `probe_err=4` / `probe_data=0`; the next Actions artifact tries the already-powered Seeed-style high/dedicated `SD.begin(13, 1000000, SPI1)` path before raw probes. This is firmware-side, not a card-format task. | Actions-built bridge artifacts flashed locally; `artifacts/hardware/com16/rp2040_direct_probe_4d691b3_actions_28605782601_COM16.json` reports `state=no_card`, `probe_err=4`, `formats_sd=false`, and all diag paths rejected at `*_e=4` |
