...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Actions run `28618724956` for `b8f3f73` proved the busy-low parser build passes Actions and keeps the bridge responsive, but direct COM16 USB still reports `state=no_card` / `probe_err=4`. The completed diagnostic shows `_c0r=0` and `_c8r=0`, so the selected card-response line is held low before command bytes; the current firmware follow-up extends SD power off/on settle and selected-card ready wait timing. This is firmware-side, not a card-format task. | Actions-built bridge artifact flashed locally; `artifacts/hardware/com16/rp2040_direct_preflight_b8f3f73_actions_28618724956_COM16.json` reports `ready_for_sd_acceptance=false`, `formats_sd=false`, `public_rf_tx=false`, accepted SPI pins, CS-high idle `255`, and selected-card ready bytes stuck at `0`, with no ESP32 flash because the only ESP32-S3 port currently enumerated is forbidden `COM11` |
