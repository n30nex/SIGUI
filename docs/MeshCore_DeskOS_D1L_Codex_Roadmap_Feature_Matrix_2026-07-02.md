...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Online research matched Seeed's official D1L/RP2040 MicroSD path and Arduino-Pico's SPI1 `SD.begin`/SdFat setup; Actions run `28623802315` for `e91c6e7` changed the failure from CMD8 echo rejection to a stricter CMD0 reset-entry rejection (`probe_err=3`, `probe_data=0`), and Actions run `28635381685` for `52e4cd9` matched Arduino-Pico's `setCS`/`setMISO`/`setMOSI`/`setSCK` SPI1 method names but still failed CMD0. Direct COM16 USB shows `detect=low`, `detect_driven=1`, accepted SPI pins including CS, CS-high idle `255`, and `hd_c0=0`. This is firmware-side SPI/power/select initialization work, not a card-format task; the next diagnostic is a bit-banged raw probe to bypass SPI1 hardware. | Actions-built bridge artifacts flashed locally; `artifacts/hardware/com16/rp2040_direct_preflight_e91c6e7_actions_28623802315_COM16.json`, `artifacts/hardware/com16/rp2040_direct_diag_poll_e91c6e7_actions_28623802315_COM16.json`, `artifacts/hardware/com16/rp2040_direct_preflight_52e4cd9_actions_28635381685_COM16.json`, and `artifacts/hardware/com16/rp2040_direct_diag_poll_52e4cd9_actions_28635381685_COM16.json` report `ready_for_sd_acceptance=false`, `formats_sd=false`, `public_rf_tx=false`, `detect=low`, `detect_driven=1`, `probe_err=3`, `probe_data=0`, and COM11/COM29 not opened |
