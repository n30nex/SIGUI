...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Actions run `28604864713` fixed the Arduino `SD`/`SDFS` wrapper wedge and kept status/diag responsive; the next Actions artifact probes the already-powered Seeed-style high bus before rail force-cycling and accepts `CMD0=0x00` only when the following SD v2 `CMD8` echo is valid. This is firmware-side, not a card-format task. | Actions-built bridge artifacts flashed locally; `artifacts/hardware/com12/rp2040_preflight_3f7e2ab_actions_28604864713_COM12.json` reports `ready_for_sd_acceptance=false`, `formats_sd=false`, `storage_diag_ok=true`, and `sd_probe_rejected_card` |
