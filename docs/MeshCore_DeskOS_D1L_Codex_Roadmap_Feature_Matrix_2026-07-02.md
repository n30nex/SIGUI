...
| A. FAT32 boot auto-provision | Partially complete | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards. Needs hardware proof with the fresh FAT32 32 GB validation card. | Host protocol/simulator tests pass; hardware not rerun yet |
...
| P0. RF receive corrupts visible UI | Complete in code/host tests; hardware proof pending | Added static mutex guards around packet, Public message, DM, node, route, contact, and mesh-inspector stores so UI snapshots and serial diagnostics cannot copy mutable rings while RF receive appends/update them. Periodic chrome refresh still avoids content redraw. | `python -m pytest tests/test_ui_shell_contract.py tests/test_ui_simulator.py tests/test_node_store_contract.py tests/test_packet_log_contract.py tests/test_mesh_inspector_contract.py tests/test_route_store_contract.py tests/test_dm_store_contract.py tests/test_contact_store_contract.py -q` -> 52 passed; `python -m pytest tests -q` -> 240 passed |
| F. Release gate | Pending | Requires current Actions artifacts/checksums, packaged notices, SD matrix, RF acceptance, manual photo review, and 12h soak. SD gate now rejects obsolete format-capable SD preflight evidence and reports FAT32-on-computer guidance only. | `python -m pytest tests/test_release_gate_audit_d1l.py -q` -> 10 passed; `python .\scripts\release_gate_audit_d1l.py --out artifacts\release-gate\d1l-release-gate-audit-phase-d-map-tiles-local.json` -> `ready_for_public_release=false`, 6 P0 failures |
...
- This is a code/host-test fix only until a downloaded GitHub Actions artifact is flashed and exercised with live packets, tab abuse, scroll probes, and manual photo review.
...
- The latest local audit `artifacts/release-gate/d1l-release-gate-audit-phase-d-map-tiles-local.json` still fails closed with six P0 blockers: missing downloaded Actions artifacts/checksums, missing packaged notices in the local audit context, SD acceptance matrix, 12-hour soak, manual physical UI/photos, and full RF acceptance.

## Next Work Order

1. Use GitHub Actions artifacts only for firmware build validation, then run COM12 hardware smoke/manual photo review against the downloaded package.
2. Complete SD acceptance with the fresh FAT32 32 GB card using a downloaded Actions artifact: prove boot auto-provision, no-card NVS fallback, non-FAT32 non-destructive guidance, and 24h packet-history retention.
3. Hardware-validate Phase D map work from a downloaded Actions artifact: Wi-Fi connect, provider setup, center-tile download, visible attribution, no public OSM bulk endpoint, and SD cache reboot/remount behavior.
