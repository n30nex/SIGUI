...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Actions run `28604061282` proved the Arduino `SD`/`SDFS` wrapper build starts cleanly and then wedges status/diag replies during `storage mount`; the next Actions artifact keeps the bridge responsive by running the bounded raw probe matrix before any filesystem mount, then mounting only raw-present candidates. This is firmware-side, not a card-format task. | Actions-built bridge artifacts flashed locally; `artifacts/hardware/com12/rp2040_preflight_4064823_actions_28604061282_COM12.json` reports `ready_for_sd_acceptance=false`, `formats_sd=false`, and post-mount `ESP_ERR_TIMEOUT` |
