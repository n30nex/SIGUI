...
1. Complete SD acceptance with a FAT32 card prepared on a computer using a downloaded Actions artifact: prove boot auto-provision, no-card NVS fallback, non-FAT32 non-destructive guidance, and 24h packet-history retention. The current COM12 preflight reports `raw_card_present_mount_failed` and `prepare_fat32_card_on_computer_or_swap_known_good_sd_card`.
