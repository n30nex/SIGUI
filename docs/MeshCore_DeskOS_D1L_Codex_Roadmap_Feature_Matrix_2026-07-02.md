...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails the bridge filesystem mount while the raw SPI probe detects the card. The next Actions artifact forces SdFat SPI command CRCs with `USE_SD_CRC=1` and force-cycles the selected SD rail before init because the current failure moved from `ACMD41` CRC rejection to `CMD0` card reset after CRC was enabled. This is firmware-side, not a card-format task. | Actions-built bridge artifacts flashed locally; COM12/COM16 evidence still reports `raw_card_present_mount_failed` / `not_fat32_or_unmountable` with `formats_sd=false` |
