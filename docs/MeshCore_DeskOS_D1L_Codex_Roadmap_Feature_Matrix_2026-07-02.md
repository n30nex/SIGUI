...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Actions run `28608728222` proved the stale `mount_pending` overwrite is fixed, but the mounted artifact still reports `probe_error=4` for the confirmed FAT32 card. The current firmware follow-up removes custom SdFat/SPI compile overrides so Actions builds with the Arduino-Pico default SD library settings used by Seeed's sample. This is firmware-side, not a card-format task. | Actions-built bridge artifacts flashed locally; `artifacts/hardware/com12/rp2040_preflight_a2be32d_actions_28608728222_COM12.json` reports final `sd.state=no_card`, `probe_error=4`, `formats_sd=false`, with no Public RF |
