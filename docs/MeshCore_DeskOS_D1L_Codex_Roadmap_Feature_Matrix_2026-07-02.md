...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. The next Actions artifact routes filesystem mount/file operations through Arduino `SD`/`SDFS`, forces SdFat SPI command CRCs with `USE_SD_CRC=1`, disables RP2040 SdFat array transfers with `USE_SPI_ARRAY_TRANSFER=0`, force-cycles the selected SD rail before init, and tightens raw probes so only a valid `CMD0` idle response reports card presence. This is firmware-side, not a card-format task. | Actions-built bridge artifacts flashed locally; COM12/COM16 evidence still reports SD not ready with `formats_sd=false` |
