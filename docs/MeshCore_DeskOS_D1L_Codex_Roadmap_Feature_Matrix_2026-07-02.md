...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Actions run `28613020379` for `998e835` proved the SPI1 reset/pin-acceptance build passes Actions and keeps the bridge responsive, but direct COM16 USB still reports `state=no_card` / `probe_err=4`. The completed diagnostic shows all SPI1 pins accepted, CS-high idle reads `255`, and all command responses still return `CMD0=0`, `CMD8=0`, and R7 `[0,0,0,0]`, so the current firmware follow-up keeps GPIO13 under software CS control instead of binding SPI1 hardware CS. This is firmware-side, not a card-format task. | Actions-built bridge artifact flashed locally; `artifacts/hardware/com16/rp2040_direct_preflight_998e835_actions_28613020379_COM16.json` reports `ready_for_sd_acceptance=false`, `formats_sd=false`, `public_rf_tx=false`, accepted SPI pins, CS-high idle `255`, and all-zero command responses, with no ESP32 flash because the only ESP32-S3 port currently enumerated is forbidden `COM11` |
