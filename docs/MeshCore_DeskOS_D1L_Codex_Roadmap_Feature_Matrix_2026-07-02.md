...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Online research matched Seeed's official D1L/RP2040 MicroSD path and Arduino-Pico's SPI1 `SD.begin`/SdFat setup; Actions run `28635381685` for `52e4cd9` matched Arduino-Pico's `setCS`/`setMISO`/`setMOSI`/`setSCK` SPI1 method names but still failed CMD0, and Actions run `28635801490` for `fcfb0f7` added a bit-banged raw diagnostic that failed the same way as hardware SPI. Direct COM16 USB shows `detect=low`, `detect_driven=1`, accepted SPI pins including CS, CS-high idle `255`, hardware SPI `hd_e=3` / `hd_c0=0`, and bit-banged `bb_e=3` / `bb_c0=0`. This is firmware-side power/select/reset-entry work, not a card-format task; the next fix floats SD SPI pins during forced GPIO18 rail-off so CS/MOSI/SCK cannot backfeed the card, then re-biases and settles before Seeed/Arduino-Pico init. | Actions-built bridge artifacts flashed locally; `artifacts/hardware/com16/rp2040_direct_preflight_52e4cd9_actions_28635381685_COM16.json`, `artifacts/hardware/com16/rp2040_direct_diag_poll_52e4cd9_actions_28635381685_COM16.json`, `artifacts/hardware/com16/rp2040_direct_preflight_fcfb0f7_actions_28635801490_COM16.json`, and `artifacts/hardware/com16/rp2040_direct_diag_poll_late_fcfb0f7_actions_28635801490_COM16.json` report `ready_for_sd_acceptance=false`, `formats_sd=false`, `public_rf_tx=false`, `detect=low`, `detect_driven=1`, `hd_c0=0`, `bb_c0=0`, and COM11/COM29 not opened |
