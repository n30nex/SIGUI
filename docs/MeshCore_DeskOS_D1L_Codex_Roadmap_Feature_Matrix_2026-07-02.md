...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Actions run `28608112968` proved the bridge no longer wedges, but the mounted artifact still reports `probe_error=4` for the confirmed FAT32 card and exposed a stale `mount_pending` cache overwrite. The current firmware follow-up keeps first mount off manual CS setup and prevents protocol-core mounts from publishing worker-only pending snapshots. This is firmware-side, not a card-format task. | Actions-built bridge artifacts flashed locally; `artifacts/hardware/com12/rp2040_preflight_d29e648_actions_28608112968_COM12.json` reports `storage_mount.sd.state=no_card`, `probe_error=4`, `formats_sd=false`, with no Public RF |
