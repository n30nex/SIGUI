...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Actions run `28618188991` for `b066f4e` proved the software-CS build passes Actions and keeps the bridge responsive, but direct COM16 USB still reports `state=no_card` / `probe_err=4`. The completed diagnostic still returns `CMD0=0`, `CMD8=0`, and R7 `[0,0,0,0]`, so the current firmware follow-up ignores leading busy-low zero bytes while waiting for CMD0/CMD8 R1 responses and reports `_c0r`/`_c8r` ready-byte diagnostics. This is firmware-side, not a card-format task. | Actions-built bridge artifact flashed locally; `artifacts/hardware/com16/rp2040_direct_preflight_b066f4e_actions_28618188991_COM16.json` reports `ready_for_sd_acceptance=false`, `formats_sd=false`, `public_rf_tx=false`, accepted SPI pins, CS-high idle `255`, and all-zero command responses, with no ESP32 flash because the only ESP32-S3 port currently enumerated is forbidden `COM11` |
