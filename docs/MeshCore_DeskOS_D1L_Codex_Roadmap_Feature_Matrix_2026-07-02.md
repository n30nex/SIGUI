...
| A. FAT32 boot auto-provision | Blocked by firmware mount issue | RP2040 mount path creates `/deskos` folders/manifests on mounted FAT32 cards, but the user-confirmed FAT32 32 GB validation card still fails before the ready file-operation gate. Actions run `28607365698` proved the protocol-core filesystem mount still times out on COM12. The current firmware follow-up removes explicit SPI CS registration and extra SdFat probing around the first Seeed-style `SD.begin(13, 1000000, SPI1)` attempt. This is firmware-side, not a card-format task. | Actions-built bridge artifacts flashed locally; `artifacts/hardware/com12/rp2040_preflight_1303ebf_actions_28607365698_COM12.json` reports `state=sd_mount_pending`, `sd_last_error=ESP_ERR_TIMEOUT`, `formats_sd=false`, with no Public RF |
