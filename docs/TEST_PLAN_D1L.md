...
6. If a long hardware validation session reports `ESP_ERR_NVS_NOT_ENOUGH_SPACE`, stop persistence mutations and capture `storage status`, retained-store counters, `settings get`, and `health`. Treat the condition as a release-blocking durability failure; do not manually erase NVS, settings, identity, contacts, or current history to make the test pass. Current candidates must report `retained_nvs.partition="d1l_retained"`, `marker_ready=true`, `ready=true`, `init_error="ESP_OK"`, and `migration_error="ESP_OK"`; retained writes go to that dedicated 124 KiB partition. A separate 4 KiB metadata sector carries two redundant version markers. First use is accepted only when both marker slots and every byte of the new retained region are already erased; the firmware initializes that blank region without a recovery erase. A corrupt marker, a missing marker beside nonblank retained data, or an unknown future marker version fails closed. On upgrade, known legacy retained keys are probed read-only and may be erased from default NVS only after their same-key dedicated copies commit. Identical duplicate copies may be reclaimed, but divergent copies are both preserved and make migration fail closed. No whole-default-NVS erase exists. Resume other recovery only with a firmware fix or a separately authorized, documented factory-reset/recovery test.
