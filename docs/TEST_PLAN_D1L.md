...
- Learned-route selection contract: a direct path is eligible only when its canonical encoding is known, it was authenticated during the current boot, and its unsigned age is no greater than 1,800,000 ms; a known zero-hop path is valid under the pinned MeshCore rule. Missing, preboot, stale, and malformed path state must produce a complete flood plan. The selected path is copied into one immutable result before transmission, ACK planning uses the same selector, and route telemetry records that actual result instead of rereading mutable contact state. PR #99 banks this software boundary in `docs/completion/evidence/wp04/route_selection_d5acf0a80af4ff533f48105ea84844bd0d9af6c3.json` (SHA-256 `e38a442100cf27f433777eaf6d293b40c44e08a555e406fd7b5d0bb79f9459ac`). Canonical TRACE session/response behavior, controlled-peer RF, reboot/power-loss, and physical qualification remain open; `codex/wp04-trace-discovery` is local implementation with no pull request yet.
...
PRs #62, #64, #80, and #84-#99 are merged on verified checkpoint `d5acf0a80af4ff533f48105ea84844bd0d9af6c3`. Exact merged-main Actions `29317376691` passed 944 host plus 32 checksum-contract tests and the Actions-only ESP32 build; all five downloaded API ZIPs and 46 manifest entries strict-passed across 219 files / 72,613,846 bytes, with independent gate receipt SHA-256 `db83f48167b57179d2ef0c0800898db51d7d24b3da4847df2af69a77344e7f61`. The production/oracle/signed-runtime Ed25519 boundary reports 864 vectors, 100,000 fuzz inputs, 215 reviewed transformations, 256 byte-identical differential cases, `full_ubsan_clean=true`, and zero exceptions/errors. PR #98's ACK aggregate SHA-256 is `351433586b62cdb4761d41a0dee146c8af4f0edd3c3b6e6e897228ea22e00f3c`; PR #99's route-selection aggregate SHA-256 is `e38a442100cf27f433777eaf6d293b40c44e08a555e406fd7b5d0bb79f9459ac`. Exact-head Actions dispatch `29296995585` remains the pinned RP2040 build checkpoint, with 9 manifests / 81 entries. The complete WP-02 runtime integration baseline remains `4ee07caf` / `29290978741`; it still fails closed only for five absent frozen-candidate physical roles. WP-03 comparison acceptance passed: exact-source full-release runs `29300805114` and `29300806682` each strict-verified 9 manifests / 89 entries / 257 files, and exact comparator receipt `babb5d8c42133ab2e0d42fc38633fbba9976c17cfd42de1eb05b5559253ac11f` reports `reproducible=true` with no failures. Preserve `a03bdb8112bc309a64ea185d2d29e7a09a8e41a282dd8fc22e2b466c9218a31e` as fixed negative history; use the tracked portable aggregate at `docs/completion/evidence/wp03/release_reproducibility_e79fb56160914f4483515f4f70998aa2f8961496.json` (SHA-256 `ff97327ae7a6c7e90f2db8905ffe344dbe73d0fb75065bbf6f66294b5c72e264`) for closure. `codex/wp04-trace-discovery` remains local implementation with no pull request, so TRACE, admin, persisted identity, RF, and physical closure remain open.
