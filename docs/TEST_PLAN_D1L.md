...
Hardware success must include a current COM12 `ui_capture_d1l.py` PNG for display pixels with a passing `ui_capture_simulator_diff` against the matching simulator/reference view, a current COM12 `ui_compose_keyboard_capture_d1l.py` artifact with Public short/long and DM short/long PNG/RGB565 captures, plus manual touch confirmation until touch automation is expanded.
