...
Use `docs/D1L_SD_CARD_GUIDED_INSTALL.md` when a public operator needs the
single guided path that downloads or accepts Actions artifacts, flashes COM12,
guides COM16 through official smoke and bridge restore, and emits
`d1l-guided-sd-install-<sha7>-actions<run-id>.json`.
...
16. On touch hardware, open Map or Settings > Map Tiles, enter the same allowed provider template and attribution, adjust zoom if needed, and verify the sheet reports live download unavailable with `tile_render_pending` instead of exposing a touch `Download` action. Use the serial `storage map-tile-download <z> <x> <y> <url-template> <attribution>` primitive for exact center-tile SD cache validation until tile rendering proof exists. Verify `settings get` reports `map_tiles.provider_saved=true`, visible attribution remains available, no Public RF is sent, no format action occurs, and the cached tile/attribution survive reboot/card remount after serial validation. The diagnostic export bundle reports map tile cache readiness but does not bundle actual tiles, and `storage export-data <token>` covers sampled user data rather than map tiles.
...
19. For the guided public install proof, run `python .\scripts\guided_sd_install_d1l.py --download-artifacts --github-run-id <run-id> --github-run-dir artifacts\github\<run-id>-current --commit <sha> --d1l-port COM12 --rp2040-port COM16`. Success requires `esp32_flash_ok=true`, `official_smoke_ok=true`, `bridge_ping_ok=true`, `ready_for_sd_acceptance=true`, at least three passing SD canary results, `public_rf_tx=false`, and `formats_sd=false`.
