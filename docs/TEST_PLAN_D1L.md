...
The runner requires no manual touch confirmation, does not open COM8, COM11, or COM29,
copies only Actions-built RP2040 UF2 artifacts, captures the RP2040-unavailable
fallback window after official SD smoke, restores the production bridge, runs
raw diagnostics plus file/export/map/retained/reboot SD canaries, and writes the
final release-gate audit under `artifacts\release-gate`.
...
Hardware success must include a current COM12 `ui_capture_d1l.py` PNG for display pixels, a current COM12 `ui_compose_keyboard_capture_d1l.py` artifact with Public short/long and DM short/long PNG/RGB565 captures, plus manual touch confirmation until touch automation is expanded.
