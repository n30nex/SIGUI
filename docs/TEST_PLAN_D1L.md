...
- Retained-worker remount contract: serial `storage remount` must acquire the storage-manager sequence before a bounded owner-safe retained-worker quiesce, exclude periodic and forced Public/DM/packet/route persistence throughout the bridge remount, release retained-worker quiesce before the manager sequence, and report `retained_worker_quiesce_acquired=true`. Acquisition failure must return before any remount/bridge operation. Reboot keeps one deadline and the order storage-manager quiesce, forced retained flush, retained-worker quiesce, then RP2040 bridge quiesce; failure unwinds in reverse, while success reports `retained_worker_quiesced=true` and holds all three quiesces through restart. SD boot-prepare acceptance must fail immediately on remount receipts without the acquired flag and on any retained degraded/backup-degraded flag, per-store SD/NVS counter, non-`ESP_OK` retained error, or degradation latch, including the existing-data scenario.
