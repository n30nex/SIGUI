...
- P0 UI hardware-script contract: `scripts/ui_corruption_probe_d1l.py --dry-run --rounds 20` and `scripts/scroll_probe_d1l.py --dry-run --screens home,public_messages,dm_thread,nodes,packets,settings,storage,wifi,map` must stay host-only, require an explicit port for hardware mode, and must exercise `ui status`, `ui tab <home|messages|nodes|map|packets|settings>`, retained data refresh via `storage retained-canary`, Packet search, Public search, telemetry fields from `health`, and crashlog checks without hardcoded COM ports. The UI corruption artifact must prove at least 20 targeted rounds, monotonic uptime, zero failures, retained data refreshes, `public_rf_tx=false`, `formats_sd=false`, and heap/LVGL/UI-stack/reset telemetry; the scroll artifact must identify each canonical surface and the tab used to reach it.
