...
7. On exact Actions-built COM12 firmware with retained packet/route data still populated, capture the three read-only aliases `ui scroll-probe mesh_roles`, `ui scroll-probe mesh_rooms`, and `ui scroll-probe mesh_repeaters`. The probe timeout must accommodate rebuilding a full Packet view without timing out, and static nested pages must return before forcing a synchronous full-screen refresh. Verify capture metadata reports matching firmware/host CRCs, `public_rf_tx=false`, and `formats_sd=false`.
