...
- SD-card data storage is entirely optional for end users. Status: ESP32-side `storage status`/`storage setup`/`storage diag`, touch Storage Setup surfaces, host protocol simulator, a guarded confirmed-format path, GitHub Actions-built RP2040 bridge firmware, bounded `DESKOS_SD_FILE v=1` file operations, SD-capable retained Public/DM message history, route history, packet history, diagnostic exports, sampled user-data exports, a first offline Map tab/cache policy, and a map-tile cache canary are implemented with onboard fallback and no boot-time or incidental formatting.
- SD bridge hardware update 2026-07-01: the RP2040 UF2 volume mounted once as `RPI-RP2` at `G:\`, and Actions artifact `28494746866` was copied with SHA-256 `05A2D728EC5EC59875C67EC2EC926B31F6032DAB1CBCD88DCA19AF54E655CC94`. Post-copy COM12 preflight then failed safely as `state="rp2040_protocol_pending"` with `storage status` and `storage diag` timeouts, no formatting, and no Public RF. Commit `e05264098a106f1ba0adcf766a1262d12e73448c` changes the RP2040 bridge to probe raw SdFat card presence before any FAT mount and adds USB serial protocol debugging; GitHub Actions run `28495545520` passed and produced verified UF2 SHA-256 `AFB6B12EE3518C48811F6C2876717B9BFAF43C1ABFE02E9BF693D95F977E16E5`. That fixed UF2 is not flashed yet because no UF2 mass-storage volume is mounted and `COM16` is access-denied/hanging, so the next step is physical RP2040 BOOTSEL/replug/power-cycle, guarded UF2 copy, and COM12 preflight before any optional format or SD acceptance canary.
