...
- Optional SD-card data layer: boot detection, filesystem validation, user-confirmed format flow, NVS fallback, generic RP2040 file operations, and SD-backed message/packet/route/map-tile stores.
...
- Final test report. Status: hardware validation notes are current through the 1-hour active Public soak, release package checkpoint, release-package sanity smoke, and COM12 `c0c20a2` evidence from Actions run `28593151178`: downloaded artifact checksum verification, esptool-verified flash, current smoke, 100-cycle tab abuse, scroll probe, and RP2040/SD preflight. Later RP2040-only Actions artifact tests kept the bridge non-formatting and responsive but still failed to mount the user-confirmed FAT32 validation card, so the SD matrix remains open as a firmware-side mount blocker. `scripts/release_gate_audit_d1l.py` still produces a fail-closed release gate artifact with `ready_for_public_release=false` while P0 evidence is missing. Final full-release report is pending the SD matrix, outbound/full RF proof, 12-hour idle/listening soak, and manual UI review/photos.
