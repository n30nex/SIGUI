...
Release status: the expanded exact-main Actions dry audit reports `ready_for_public_release=false`, with **33 remaining P0 failures and 35 failures overall including P1**; the current audit receipt SHA-256 is `83223d50b78044b497e5db43f4dba00353b535c43a978613318de73c23fdeca8`. `scripts/release_gate_audit_d1l.py` must report `ready_for_public_release=true` for the exact release commit before tagging; older evidence or individually passing branches cannot close that gate.

Execution checkpoint: live `main` is `d5acf0a80af4ff533f48105ea84844bd0d9af6c3` after PR #98 merged schema-v5 durable inbound-DM ACK handling and PR #99 merged freshness-gated learned-route selection. Exact merged-main Actions `29317376691` passed 944 host tests plus 32 checksum-contract tests and the Actions-only ESP32 build; all five API ZIPs and 46 nested entries strict-passed across 219 files / 72,613,846 bytes, and the independent merged-main gate receipt SHA-256 is `db83f48167b57179d2ef0c0800898db51d7d24b3da4847df2af69a77344e7f61`. PR #98's portable ACK aggregate is `docs/completion/evidence/wp04/ack_durability_76b07f28918b338bf896d5a1a8a0207b5a112677.json` (SHA-256 `351433586b62cdb4761d41a0dee146c8af4f0edd3c3b6e6e897228ea22e00f3c`); PR #99's route aggregate is `docs/completion/evidence/wp04/route_selection_d5acf0a80af4ff533f48105ea84844bd0d9af6c3.json` (SHA-256 `e38a442100cf27f433777eaf6d293b40c44e08a555e406fd7b5d0bb79f9459ac`). The merged route selector accepts immutable current-boot fresh paths, including a known zero-hop path, and fails closed to flood for missing, preboot, stale, or malformed paths; ACK planning reuses that selector and route telemetry records the actual immutable send plan. WP-04 remains in progress: `codex/wp04-trace-discovery` is a local implementation branch with no pull request yet, and no TRACE, admin, persisted-identity, RF, or physical closure is claimed. WP-01 remains banked on `092293f`; WP-02 physical qualification remains open for five frozen-candidate roles. [COMPLETION_LEDGER.yaml](COMPLETION_LEDGER.yaml) remains the machine-readable execution source, with [COMPLETION_STATUS.md](COMPLETION_STATUS.md) as its generated view.
...
1. PRs #62, #64, #80, and #84-#99 are merged on exact `main` `d5acf0a80af4ff533f48105ea84844bd0d9af6c3`; WP-03 reproducibility proof is banked, WP-04 production/oracle Ed25519, schema-v5 ACK durability, and learned-route selection software boundaries are merged, and the exact IDF-version receipt defect is closed.
2. Exact merged-main Actions `29317376691` built the applicable ESP32/package inputs and strict-passed 3 downloaded manifests / 46 entries; the full RP2040-inclusive integration baseline remains `4ee07caf` / `29290978741` at 8 manifests / 78 entries. Continue WP-04 with the local `codex/wp04-trace-discovery` implementation while the five WP-02 physical roles remain explicitly deferred to the frozen final candidate; green builds do not substitute for TRACE, RF, or physical proof.
...
WP-01's exact-pair proof is banked. The integration, release-security, WP-04 Ed25519, exact-IDF-receipt, schema-v5 ACK durability, and learned-route selection slices through PR #99 are merged on live `main` `d5acf0a80af4ff533f48105ea84844bd0d9af6c3`; exact merged-main Actions/checksums and WP-03 reproducibility proof pass. The tracked, repository-relative WP-02 baseline at `docs/completion/evidence/wp02/integration_baseline_4ee07caf09906abdcebe8faccd95790dceb5fe88.json` has SHA-256 `39d8632d6de5bc819a96e92e970b9d280130a3014336be5d045a1f3fe07b654c` and fails closed only on five missing physical roles. Keep that receipt open without stalling WP-04, and collect those roles once on the frozen final candidate. The active `codex/wp04-trace-discovery` work is local implementation with no pull request yet; TRACE, admin, identity, RF, and physical closure remain open. The earlier PR #80 `341a3abf4db4c52acf5859e396f25e7adb4cbab1` result is rehearsal history only.
