...
|---|---|---|
| UI modular ownership and newest Home proof | Navigation ownership, the modal helper boundary, shared chrome/layout policy, and compose-probe keyboard target/geometry policy are split into `ui_navigation.c`, `ui_modal.c`, `ui_chrome.c`, and `ui_keyboard.c`. The remaining #16/#6 work is to finish owned screen/render modules, residual keyboard widget wiring, single-active-root invariants, and current Home pixel/manual evidence without broad UI cycling. | For code-only module slices, use host tests plus default Actions. For Home/pixel acceptance, run only the matching COM12 `ui_capture_d1l.py` proof once COM12 is available. |
