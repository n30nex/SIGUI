...
- Optional SD-card data storage. Status: important production feature with ESP32-side RP2040 SD query/setup plumbing implemented. The current D1L ESP32 BSP exposes no direct SDMMC/SDSPI card pins, so `storage status` now uses a short `DESKOS_SD_STATUS` RP2040 UART probe, reports whether the bridge protocol/card/filesystem/root are ready, and keeps all retained stores on onboard NVS/serial fallback. `storage setup` and the Settings Storage Setup sheet expose the next safe action, but no automatic format is performed; even the confirmation phrase remains non-destructive until the RP2040 bridge implements the matching command. Next work should add the RP2040 firmware side of the card-status/filesystem/format protocol, then move larger retained stores to SD first: Public/DM message history, packet/route history, diagnostic exports, and future map-tile caches. Settings, identity, and minimum boot-critical state should stay safe in onboard storage so the device remains usable without a card.
...
- SD-card data storage is entirely optional for end users. Status: ESP32-side `storage status`/`storage setup` and touch Storage Setup surfaces are implemented with onboard fallback and no automatic format. Full acceptance still requires the RP2040 bridge firmware to answer the protocol, boot to remain normal with no card, bad/unformatted cards to be detected without blocking normal operation, formatting to require explicit user confirmation, and SD-backed stores to survive reboot and card remount when configured.
