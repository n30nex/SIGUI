...
- SD bridge hardware update 2026-07-01: Actions run `28502095977` for commit `552a59df3a5810c5004f496584020d31f5f1dea4` flashed the RP2040 UF2 with SHA-256 `3C27ABCDCBE525E6F305F39C37C413ED00CC0DF45A3DA3D7424E6EC69B5DF7A7`. COM12 preflight `artifacts/hardware/com12/rp2040_sd_preflight_raw_probe_28502095977.json` proved `rp2040 ping` works, safe `storage status` reports `state="mount_required"` without touching SD, explicit `storage mount` is responsive, and the bounded raw probe detects the inserted card as `setup_required`/`format_supported=true` without Public RF or formatting. A guarded format attempt (`artifacts/hardware/com12/sd_format_guarded_28502095977.json`) still left the card `setup_required`, so the file-operation gate and canaries remain pending; the current follow-up makes non-ready `FORMAT_REPLY` results report as failed instead of `format_performed=true`.
