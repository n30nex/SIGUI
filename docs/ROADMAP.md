...
- Optional SD-card data layer: boot detection, FAT32 filesystem validation, no-format NVS fallback for unusable cards, generic RP2040 file operations, and SD-backed message/packet/route/map-tile stores.
...
- Public chat screen. Status: first touch Public screen is implemented with persisted recent message rows, a paged retained Public History sheet, Public search entry sheet, Load Older, and `messages public [offset <n>]` / `messages public search <text> [offset <n>]` serial filtering; the older non-paged slice was host/simulator covered, flashed on `COM7`, standard-smoked without Public-channel RF transmit, and hardware-regression checked through a targeted COM11 direct-message proof. Newest paging behavior is host-tested and awaits Actions-artifact COM12 proof.
- DM screen. Status: first persisted DM preview is implemented in the Messages screen, prerequisite full public-key retention from signed adverts into heard nodes and promoted contacts is implemented and validated on `COM7`, a first contact-row touch DM compose flow is implemented/built/flashed, and a paged DM thread/detail sheet opens from recent DM rows with Load Older, `Reply`, per-thread `Read`, and scrollable retained thread history; manual touch review is still pending.
...
- Unread/mute/favorite logic. Status: first contact favorite/mute flags are editable through serial diagnostics and the touch contact detail sheet, and favorite/mute persistence across reboot is validated on `COM7`. First persisted Public/DM read-state cursors, bounded per-thread DM read cursors, `messages unread`, `messages read <public|dm|dm <fingerprint>|all>`, Messages-screen unread counters, unread row highlighting, a global touch `Read` action, and a DM-thread `Read` action are implemented. Public unread/read persistence is validated on `COM7` with a fresh Public `test` RX window; controlled inbound DM unread proof is pending.
...
- Contact detail cards. Status: first bounded persisted contact store, `contacts` diagnostics, heard-node promotion command, full advertised public-key retention, first contact detail sheet, favorite/mute edit actions, and serial/touch contact rename/delete management are implemented; richer detail cards and manual touch review are pending.
- Route/path metadata. Status: first bounded persisted route store, `routes`, `routes detail <seq>`, `routes trace <fingerprint>` diagnostics, and opt-in DM-only `routes probe <fingerprint>` active trace command are implemented. MeshCore Public/advert route observation wiring, encrypted PATH return parsing, contact return-path retention, direct-route DM TX selection, Packet-tab route rows, a first route detail sheet, and a first contact Route Trace sheet with `Ping` are implemented; controlled PATH RF proof and manual touch review of route/trace detail are pending on the newest build.
- Node actions: DM, ping/trace if available, telemetry request if available. Status: serial `mesh send dm <fingerprint> <text>`, first touch contact-row `DM` action, contact-detail `DM` action, contact-detail `Trace`, and a DM-only active `Ping`/`routes probe` action are implemented for promoted contacts with retained public keys. Telemetry actions are pending.

Acceptance:

- User can send and receive public and DM messages from touch UI. Status: controlled Public `test` TX/RX works and persists in the touch Public view; first free-text Public composer is implemented; paged retained Public History/search with Load Older is implemented and host/simulator covered; first contact-row touch DM composer is implemented and routes through the same DM backend as serial; paged DM thread/detail history with Load Older, `Reply`, and per-thread `Read` is implemented; serial DM flood TX/store/reboot persistence is validated with `YKF Corebot`; targeted outbound DM receive by the COM11 Meshcorebot passed without Public-channel RF, and `scripts/probe_d1l_dm.py` now repeats that DM-only hardware proof by checking D1L retained DM/packet rows plus COM11 receive counters. Latest COM12 evidence `artifacts/hardware/com12/dm_probe_b841621c.json` passed with retained DM, packet-search, route-trace, COM11 meshbot receive-counter, health, and no-Public-command checks all true. Newest paging behavior still needs COM12 proof from a downloaded GitHub Actions artifact. DM ACK/PATH receive parsing and direct-route TX selection are implemented. First Public unread/read behavior is validated on `COM7`: after `messages read all`, a fresh Public `test` produced two RX rows, raised `public_unread=2`, cleared with `messages read public`, and stayed cleared after reboot. Manual physical touch entry/thread/reply/read/Public-history review, inbound DM proof, controlled ACK/PATH RF proof, and direct-route RF proof are pending.
- Node list does not crash or stutter with large simulated meshes. Status: bounded preview rows avoid unbounded UI work, and a stack-overflow boot loop found during hardware smoke was fixed by moving UI snapshot storage off the main task stack. Host simulator large-mesh stress now feeds 96 heard nodes, 18 contacts, 48 Public messages, and 32 DM messages while asserting bounded visible rows, retained Public History bounds, and no text overflow; live RF stress with a genuinely large hardware mesh is still pending.
...
- Wi-Fi scan/connect screen. Status: Phase D station runtime path is implemented behind explicit user enablement: `wifi status`, bounded `wifi scan`, `wifi save <ssid> [password]`, password-safe `wifi connect`, `wifi clear`, `wifi on|off`, and the touch Wi-Fi Setup sheet are implemented. Wi-Fi remains off by default and must be validated on hardware from downloaded GitHub Actions artifacts.
...
- BLE or Wi-Fi companion mode is documented with exact supported clients. Status: Wi-Fi station setup/scan/connect is implemented for local setup and map-tile preparation; BLE companion transport remains pending/build-disabled until measured runtime validation.
- Wi-Fi can be disabled and remains disabled across reboot if user chooses. Status: `wifi off` persists disabled state through the settings model, and schema v6 persists clearable Wi-Fi profile metadata plus optional map-tile provider metadata for the future measured runtime path.
...
- Repeaters screen. Status: first read-only serial visibility and a first touch Mesh Roles browser sheet are implemented, built, flashed, smoke-tested, reboot-checked, and Public `test` RF-regression tested on `COM7`; retained-evidence Route Trace and an opt-in DM-only active trace probe are implemented.
...
- Radio settings editor. Status: first touch Radio Settings sheet is implemented from the Settings tab with staged frequency/BW/SF/CR/TX power/RX boost controls, a US/CAN defaults action, explicit Save, dynamic RF apply status, app-model validation, and serial `radio set txpower`/`radio set rxboost` coverage; host simulator, contract tests, GitHub Actions build artifacts, COM7 flash, standard smoke, targeted serial radio proof, and active Public `test` RF regression pass. Manual touch review is pending.
- Optional SD-card data storage. Status: important production feature with ESP32-side RP2040 SD query/setup plumbing, a CI-buildable RP2040 SD bridge target, generic bounded `DESKOS_SD_FILE v=1` file operations, and a retained blob-store abstraction. Retained Public message history, DM message history, route history, and packet history can now switch to SD when the RP2040 reports a ready FAT32 card, file operations, matching line/path/chunk limits, and atomic rename. The stores commit through `stores/messages/public/public.tmp` -> `stores/messages/public/public.bin`, `stores/messages/dm/threads.tmp` -> `stores/messages/dm/threads.bin`, `stores/routes/routes.tmp` -> `stores/routes/routes.bin`, and `stores/packet_log/ring.tmp` -> `stores/packet_log/ring.bin`; NVS remains mirrored as fallback. With no card, timeout, missing file support, missing atomic rename, insufficient limits, unmountable/non-FAT32 cards, or invalid SD blobs, retained history falls back to NVS. `storage setup` and the Settings SD Card sheet are non-destructive and report `policy="no_device_format"`; users must prepare FAT32 cards on a computer, and the RP2040 bridge has no SD formatting command. Diagnostic SD exports are implemented through `storage export-canary <token>`, `storage export-diagnostics <token>`, `scripts/sd_export_canary_d1l.py`, and `scripts/sd_diagnostic_export_d1l.py`: they write under `exports/diagnostics/` by temp write/read plus atomic rename, leave final JSON present, support chunked diagnostic bundles, report map-tile cache readiness, and report no Public RF or formatting. Sampled user-data SD exports are implemented through `storage export-data <token>` and `scripts/sd_data_export_d1l.py`: they write under `exports/data/`, include recent messages, DMs, routes, packets, contacts, nodes, settings summary, manual map-center summary, and read-state, exclude private identity material, and report no Public RF or formatting. The map-tile cache policy is implemented through `storage map-policy`, the touch Map tab, Map Tiles provider sheet, and host simulator coverage: it exposes the offline cache path `map/tiles/z{z}/x{x}/y{y}.tile`, cache readiness, `tile_render_pending` live-download state, visible-attribution requirement, public OSM bulk-download block, manual center state, persisted provider/attribution/zoom settings, and no Public RF or formatting. The map-tile cache canary is implemented through `storage map-tile-canary <token>` and `scripts/sd_map_tile_canary_d1l.py`; it commits a synthetic tile under `map/tiles/` by temp write/read plus atomic rename and leaves the final tile present for inspection. The read-only `storage map-tile-check <token>` path and `scripts/sd_reboot_remount_acceptance_d1l.py` now provide the retained-store plus map-tile reboot/remount acceptance flow; real-card hardware proof remains pending. Settings, identity, contacts, read-state, and minimum boot-critical state should stay safe in onboard storage so the device remains usable without a card.
- Optional manual location/map view. Status: first offline Map tab implemented with cache/download policy, retained route/node evidence, serial-configured manual center through `map center set <lat> <lon>`, a first-open touch latitude/longitude keyboard sheet, persisted D1L pin and map-tile provider state through the app model/settings schema v6 migration, sampled export summary, simulator coverage, a serial single-tile `storage map-tile-download` path, and a touch Map Tiles sheet that saves an allowed non-public-OSM HTTPS provider template plus attribution/zoom while live touch downloads remain unavailable as `tile_render_pending`. GPS/location-source integration, browser/geolocation integration, nearby GPS node pins, SD tile rendering/sideloading, and hardware proof remain pending; the view should use optional SD-card data for map tiles and offline map/cache data when a card is configured.

Acceptance:

- User can inspect how the mesh is behaving without using serial logs. Status: first Home/Packet-tab signal, room-server, and repeater-candidate summaries are validated on `COM7`, and contact Route Trace can summarize retained route/contact evidence from touch. The Route Trace sheet now has an opt-in DM-only `Ping` action for active ACK/PATH evidence; richer full-list screens and newest-build hardware proof are pending.
...
- SD-card data storage is entirely optional for end users. Status: ESP32-side safe `storage status`, explicit non-formatting `storage mount`, `storage setup`, `storage diag`, touch SD Card surfaces, host protocol simulator, GitHub Actions-built RP2040 bridge firmware, bounded `DESKOS_SD_FILE v=1` file operations, SD-capable retained Public/DM message history, route history, packet history, diagnostic exports, sampled user-data exports, a first offline Map tab/cache policy, and a map-tile cache canary are implemented with onboard fallback and no on-device formatting.
- SD bridge hardware update 2026-07-02: Prior guarded-format evidence is obsolete for release readiness. The current policy requires a FAT32 card prepared on a computer, automatic DeskOS folder/manifest creation on mounted FAT32 media, NVS fallback for no-card or unusable-card states, and no ESP32/RP2040/serial/script/UI formatting command. The current validation card is user-confirmed FAT32, so the remaining `raw_card_present_mount_failed` evidence is a firmware-side RP2040 mount blocker.
...
- Final test report. Status: hardware validation notes include the 1-hour active Public soak, release package checkpoint, release-package sanity smoke, and COM12 `c0c20a2` evidence from Actions run `28593151178`: downloaded artifact checksum verification, esptool-verified flash, smoke, shorter historical tab-abuse, scroll probe, and RP2040/SD preflight. The final release gate now requires a current-commit 500-cycle tab-abuse artifact plus the full scroll-surface plan. Later RP2040-only Actions artifact tests kept the bridge non-formatting and responsive but still failed to mount the user-confirmed FAT32 validation card, so the SD matrix remains open as a firmware-side mount blocker. `scripts/release_gate_audit_d1l.py` still produces a fail-closed release gate artifact with `ready_for_public_release=false` while P0 evidence is missing. Final full-release report is pending the SD matrix, outbound/full RF proof, 12-hour idle/listening soak, and manual UI review/photos.
...
- Final docs match actual behavior. Status: docs are updated through the `c0c20a2` COM12 release-gate progress slice; final audit must pass with `ready_for_public_release=true` before a production tag.
