...
|---|---|---|
| UI modular ownership and current Home proof | Navigation ownership, the modal helper boundary, shared chrome/layout policy, Home state/geometry policy, compose-probe keyboard target/geometry policy, compose/non-compose keyboard layout and binding configuration, and the single-root screen render/content-root boundary are split into `ui_navigation.c`, `ui_modal.c`, `ui_chrome.c`, `ui_home.c`, `ui_keyboard.c`, and `ui_screen.c`. PR #40 closed the prior Home pixel proof on COM12 from `b2ca00a` / Actions `28731948363`; current Home geometry has since tightened the title strip, launcher tiles, and footer status icons. The remaining #16/#6 work is to finish owned screen/render modules, the remaining keyboard event/focus wiring, current-commit Home proof when the Home surface changes, and deeper single-root invariants without broad UI cycling. | For code-only module slices, use host tests plus default Actions. For one-surface Home/pixel acceptance, run only the matching COM12 `ui_capture_d1l.py` proof. |
