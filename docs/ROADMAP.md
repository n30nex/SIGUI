...
- DM screen. Status: first persisted DM preview is implemented in the Messages screen, and the prerequisite full public-key retention from signed adverts into heard nodes and promoted contacts is implemented and validated on `COM7`; touch DM thread/compose workflow is still pending.
...
- Message delivery states. Status: persisted rows show queued TX, received RX, and DM TX ACK hash fields; ACK/delivery confirmation is still pending.
...
- Node actions: DM, ping/trace if available, telemetry request if available. Status: serial `mesh send dm <fingerprint> <text>` is implemented for promoted contacts with retained public keys; touch node actions are pending.

Acceptance:

- User can send and receive public and DM messages from touch UI. Status: controlled Public `test` TX/RX works and persists in the touch Public view; first free-text Public composer is implemented; serial DM flood TX/store/reboot persistence is validated with `YKF Corebot`; manual touch entry review, touch DM workflow, inbound DM proof, ACK handling, and direct-path routing are pending. The latest Public `test` window proved COM11 bot receipt/relay but did not produce a fresh D1L-heard `Test OK` reply.
