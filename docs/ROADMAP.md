...
- SD bridge hardware update 2026-07-01: Actions run `28549189659` for commit `b841621` produced verified ESP32 release and RP2040 SD bridge artifacts; the ESP32 package flashed to COM12 and `artifacts/hardware/com12/smoke_b841621.json` passed. The RP2040 UF2 checksum is `032FF80A0F94613BB18742E08CB97AA548BFF81BD627FF882C3AFACAF15F5C01`, but `artifacts/hardware/com12/rp2040_uf2_volumes_b841621.json` found `candidate_volumes=[]`, so the updated bridge has not been copied. `artifacts/hardware/com12/rp2040_preflight_b841621_uf2_not_mounted.json` proves the RP2040 UART/ping/protocol/diag paths respond and the inserted card reaches `setup_required` without Public RF or formatting, but `ready_for_sd_acceptance=false`; the full SD auto-prepare, retained-history, export, map-tile, and reboot/remount matrix remains blocked until the RP2040 is placed in UF2/BOOTSEL mode, flashed, and retested.
...
- Final test report. Status: hardware validation notes are current through the 1-hour active Public soak, release package checkpoint, release-package sanity smoke, COM12 outbound DM proof against the COM11 Meshcorebot, and the current SD/RP2040 UF2 blocker; final full-release report is pending the SD matrix, 12-hour idle/listening soak, and manual UI review.
