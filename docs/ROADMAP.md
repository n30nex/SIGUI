...
- Radio settings editor. Status: first touch Radio Settings sheet is implemented from the Settings tab with staged frequency/BW/SF/CR/TX power/RX boost controls, a US/CAN defaults action, explicit Save, reboot/apply warning, app-model validation, and serial `radio set txpower`/`radio set rxboost` coverage; host simulator, contract tests, GitHub Actions build artifacts, COM7 flash, standard smoke, targeted serial radio proof, and active Public `test` RF regression pass. Manual touch review is pending.
