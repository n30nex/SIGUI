...
- Optional SD-card data storage. Status: important production feature with ESP32-side RP2040 SD query/setup plumbing implemented. The current D1L ESP32 BSP exposes no direct SDMMC/SDSPI card pins, so `storage status` now uses a short `DESKOS_SD_STATUS` RP2040 UART probe, reports whether the bridge protocol/card/filesystem/root are ready, and keeps all retained stores on onboard NVS/serial fallback. `storage setup` and the Settings Storage Setup sheet expose the next safe action, and the ESP32 side now has a guarded `DESKOS_SD_FORMAT FORMAT-DESKOS-SD` request path that is only reachable after exact user confirmation plus a bridge-reported present card, `format_supported=true`, and setup-required state. The repo still has no buildable D1L RP2040 SD bridge firmware target, so current hardware remains non-destructive and falls back to NVS. Next work should add/import the RP2040 firmware target that implements `docs/SD_BRIDGE_PROTOCOL_D1L.md`, then move larger retained stores to SD first: Public/DM message history, packet/route history, diagnostic exports, and future map-tile caches. Settings, identity, and minimum boot-critical state should stay safe in onboard storage so the device remains usable without a card.
...
- SD-card data storage is entirely optional for end users. Status: ESP32-side `storage status`/`storage setup`, touch Storage Setup surfaces, host protocol simulator, and a guarded confirmed-format request path are implemented with onboard fallback and no boot-time or incidental formatting. Full acceptance still requires a buildable RP2040 bridge firmware target to answer the protocol, boot to remain normal with no card, bad/unformatted cards to be detected without blocking normal operation, formatting to execute only after explicit user confirmation and bridge support, and SD-backed stores to survive reboot and card remount when configured.
