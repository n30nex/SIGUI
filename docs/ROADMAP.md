...
Release status: the expanded exact-main audit reports `ready_for_public_release=false`, with **32 remaining P0 failures and 34 failures overall including P1**. `scripts/release_gate_audit_d1l.py` must report `ready_for_public_release=true` for the exact release commit before tagging; older evidence or individually passing branches cannot close that gate.

Execution checkpoint: live `main` is `b49a7b3a18379fdb6e4fe95c46784e8e2ea79d2e` after PR #94 merged the defined-arithmetic Ed25519 overlay foundation and PR #93 merged real signed-advert Identity/Mesh/Dispatcher/BaseChatMesh execution. Exact merged-main Actions `29307595930` passed 917 host tests plus 28 checksum-contract tests; all five artifacts matched GitHub API digests and strict-passed 3 manifests / 44 entries across 216 files / 72,353,618 bytes, with ordinal canonical inventory `9117424199086903f96138436d686a031cafcfc6636c857f0e25b5e782b68df9`. Portable aggregate `docs/completion/evidence/wp04/signed_advert_ed25519_foundations_b49a7b3a18379fdb6e4fe95c46784e8e2ea79d2e.json` has SHA-256 `0203d464868d46fde17cde13b391ac12af4f5089bae32db6cf2898776f192cef`. WP-03 remains banked; WP-04 is in progress with production/oracle Ed25519 integration and ACK dispatch actively implemented. WP-01 remains banked on `092293f`; WP-02 physical qualification remains open for five frozen-candidate roles. [COMPLETION_LEDGER.yaml](COMPLETION_LEDGER.yaml) remains the machine-readable execution source, with [COMPLETION_STATUS.md](COMPLETION_STATUS.md) as its generated view.
...
1. PRs #62, #64, #80, #84-#94 are merged on exact `main` `b49a7b3a18379fdb6e4fe95c46784e8e2ea79d2e`; WP-03 reproducibility proof is banked and WP-04 signed-advert/Ed25519 foundations are merged.
2. Exact merged-main Actions `29307595930` built the applicable ESP32/package inputs and strict-passed 3 downloaded manifests / 44 entries; the full RP2040-inclusive integration baseline remains `4ee07caf` / `29290978741` at 8 manifests / 78 entries. Continue WP-04 while the five WP-02 physical roles remain explicitly deferred to the frozen final candidate; green builds do not substitute for physical proof.
...
real signed-advert Identity/Mesh/Dispatcher/BaseChatMesh execution. The
current receipt remains `closure_ready=false`: production/oracle overlay
selection, ACK, route, TRACE, admin, retained-state, real-peer RF, and physical
proof are still open. See
...
WP-01's exact-pair proof is banked. The integration, release-security, and WP-04 foundation slices through PR #94 are merged on live `main` `b49a7b3a18379fdb6e4fe95c46784e8e2ea79d2e`; exact merged-main Actions/checksums and WP-03 reproducibility proof pass. The tracked, repository-relative WP-02 baseline at `docs/completion/evidence/wp02/integration_baseline_4ee07caf09906abdcebe8faccd95790dceb5fe88.json` has SHA-256 `39d8632d6de5bc819a96e92e970b9d280130a3014336be5d045a1f3fe07b654c` and fails closed only on five missing physical roles. Keep that receipt open without stalling WP-04, and collect those roles once on the frozen final candidate. The earlier PR #80 `341a3abf4db4c52acf5859e396f25e7adb4cbab1` result is rehearsal history only.
