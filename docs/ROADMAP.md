...
|---|---|---|
| UI modular ownership and current Home proof | Navigation, shared chrome/layout, Home state/geometry/rendering, keyboard policy, and the screen-root boundary are split into `ui_navigation.c`, `ui_chrome.c`, `ui_home.c`, `ui_keyboard.c`, and `ui_screen.c`; `ui_modal.c` remains a helper boundary rather than an exclusive manager. PR #51 closed the tightened Home pixel proof on COM12 from `485744e` / Actions `29055441069` with matching CRC `BB15A654` and simulator diff success. `ui_screen.c` now owns one cleanup/fallback dispatch and passes the canonical content root explicitly to every top-level renderer. The remaining #16/#6 work is to finish the non-Home owned screen/render modules, add modal exclusivity and UI-task command ownership, finish sheet-specific keyboard submit/cancel ownership, and deepen single-root/callback-lifetime invariants. | For pixel-neutral module slices, use host tests plus default Actions. Rerun a one-surface COM12 capture only when the Home surface changes; the final frozen release commit still needs commit-matched gate evidence. |
...
1. Continue #16/#6 UI modular ownership slices from the owned Home renderer and explicit-root contract toward owned non-Home screen modules, modal exclusivity, and UI-task command ownership.
