...
- Route/path metadata. Status: first bounded persisted route store, `routes`, `routes detail <seq>`, and read-only `routes trace <fingerprint>` diagnostics, MeshCore Public/advert route observation wiring, encrypted PATH return parsing, contact return-path retention, direct-route DM TX selection, Packet-tab route rows, a first route detail sheet, and a first contact Route Trace sheet are implemented and locally built/flashed on `COM7`; controlled PATH RF proof, manual touch review of route/trace detail, and active RF ping/trace helpers are pending.
- Node actions: DM, ping/trace if available, telemetry request if available. Status: serial `mesh send dm <fingerprint> <text>`, first touch contact-row `DM` action, contact-detail `DM` action, and a read-only contact-detail `Trace` action backed by retained route/contact evidence are implemented for promoted contacts with retained public keys; active RF ping/trace and telemetry actions are pending.
...
- Repeaters screen. Status: first read-only serial visibility and a first touch Mesh Roles browser sheet are implemented, built, flashed, smoke-tested, reboot-checked, and Public `test` RF-regression tested on `COM7`; a read-only retained-evidence Route Trace helper is implemented, while active RF ping/trace actions are pending later UX expansion.
