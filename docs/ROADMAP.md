...
- SD bridge hardware update 2026-07-01: Actions run `28549761003` for commit `68350bf` produced verified ESP32 release and RP2040 SD bridge artifacts; the downloaded firmware, release package, and RP2040 checksum manifests pass, and `artifacts/hardware/com12/smoke_b841621.json` remains the latest passing COM12 smoke. The RP2040 UF2 checksum is `032FF80A0F94613BB18742E08CB97AA548BFF81BD627FF882C3AFACAF15F5C01`, but `artifacts/hardware/com12/rp2040_uf2_volumes_68350bf_after_reset.json` found `candidate_volumes=[]` even after a safe `rp2040 reset`, so the updated bridge has not been copied. `artifacts/hardware/com12/rp2040_preflight_68350bf_after_reset.json` proves the RP2040 UART/ping/protocol/diag paths respond and the inserted card reaches `setup_required` without Public RF or formatting, but `ready_for_sd_acceptance=false`; the full SD auto-prepare, retained-history, export, map-tile, and reboot/remount matrix remains blocked until the RP2040 is placed in UF2/BOOTSEL mode, flashed, and retested.
