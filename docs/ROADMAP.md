...
- SD-card data storage is entirely optional for end users. Status: ESP32-side `storage status`/`storage setup`, touch Storage Setup surfaces, host protocol simulator, a guarded confirmed-format request path, GitHub Actions-built RP2040 bridge firmware, bounded generic file-operation protocol support, and SD-capable retained Public/DM message, route, and packet history are implemented with onboard fallback and no boot-time or incidental formatting. Full acceptance still requires the RP2040 bridge firmware to be safely flashed and validated on hardware, boot to remain normal with no card, bad/unformatted cards to be detected without blocking normal operation, formatting to execute only after explicit user confirmation and bridge support, export/map-tile stores to land, and SD-backed stores to survive reboot and card remount when configured.
