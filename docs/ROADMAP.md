...
- SD bridge hardware update 2026-07-01: Actions run `28501529179` for commit `0697d61cac2fcf7bb67a54312ec591e91b0ce363` flashed the ESP32 on COM12 and copied the RP2040 UF2 with SHA-256 `FC825D28D883656E0E786B738B1B34F4C8E7AFBD5EA7E2A72930A98722078A7E`. COM12 preflight `artifacts/hardware/com12/rp2040_sd_preflight_async_mount_28501529179.json` proved `rp2040 ping` works, safe `storage status` reports `state="mount_required"` without touching SD, and explicit `storage mount` now returns `state="mount_pending"` without Public RF or formatting. The later Arduino/SdFat mount path still stalled RP2040 status replies, so the current follow-up adds a bounded raw SPI presence probe before any filesystem mount attempt; the next hardware step is Actions build, flash/copy, COM12 preflight, and then canaries only if the mount reports a ready file gate.
