...
- SD-card data storage is entirely optional for end users. Status: ESP32-side `storage status`/`storage setup`, touch Storage Setup surfaces, host protocol simulator, a guarded confirmed-format request path, GitHub Actions-built RP2040 bridge firmware, bounded generic file-operation protocol support, SD-capable retained Public/DM message, route, and packet history, diagnostic SD exports, sampled user-data SD exports, a first offline Map tab/cache policy with serial-configured manual center, and a map-tile cache canary are implemented with onboard fallback and no boot-time or incidental formatting. GitHub Actions run `28490556715` for commit `1afd4043ce2473dddeaa66f23fafb667d2a98b88` passed host checks and produced verified ESP32/RP2040 artifacts; the ESP32 firmware was flashed on COM12 and COM12 smoke passed. The verified RP2040 UF2 SHA-256 is `05A2D728EC5EC59875C67EC2EC926B31F6032DAB1CBCD88DCA19AF54E655CC94`, but no RP2040 UF2/BOOTSEL mass-storage volume was mounted, so it has not been copied yet. Current COM12 preflight classifies the hardware as `sd_card_not_present_diag_pending` with `next_action="put_rp2040_in_uf2_bootloader"`: the old bridge answers `DESKOS_SD_STATUS` but times out on `DESKOS_SD_DIAG`, reports `sd.state=no_card`, and no formatting or Public RF occurred. Full acceptance still requires flashing the RP2040 bridge UF2, proving the inserted card through `storage diag`, detecting bad/unformatted cards without blocking normal operation, formatting only after explicit user confirmation and bridge support, map-tile cache and SD-backed stores to survive reboot and card remount when configured, and live network tile downloads after Wi-Fi runtime and explicit user opt-in.
...
- Final test report. Status: hardware validation notes are current through the 1-hour active Public soak, release package checkpoint, release-package sanity smoke, and Public History/Search plus COM11-DM checkpoint; final full-release report is pending the 12-hour idle/listening soak and manual UI review.
...
- Generate screenshots for every main screen. Status: `tools/ui_simulator.py --out artifacts/ui-sim` renders Home, Messages, Nodes, Map, Packets, Settings, compose/Public-history/Public-search/advert/radio settings/contact/DM/route/packet/mesh-role sheets, lock overlay, and first-boot onboarding; `tools/ui_simulator.py --scenario large-mesh --out artifacts/ui-sim-large` renders the same UI against oversized node/message stores, and `tools/ui_simulator.py --scenario manual-location --out artifacts/ui-sim-manual-location` renders the Map page with a persisted manual center.
- Assert required labels and button surfaces are present. Status: schema-v2 `ui-sim-report.json` now includes a touch-target map with expanded 44x44 boxes and expected flow coverage for onboarding, lock/unlock, Public compose/history/search, DM thread read/reply, contact detail/edit/export/trace, Map policy/manual center, packet search/detail/routes, Mesh Roles, Radio, Storage, and Advert paths.
