...
|---|---|---|
| UI modular ownership and current Home proof | Navigation, shared chrome/layout, Home state/geometry/rendering, the top-level Settings dashboard/actions, keyboard policy, the screen-root boundary, and single-active modal ownership are split into `ui_navigation.c`, `ui_chrome.c`, `ui_home.c`, `ui_settings.c`, `ui_keyboard.c`, `ui_screen.c`, and `ui_modal.c`. PR #51 closed the tightened Home pixel proof on COM12 from `485744e` / Actions `29055441069` with matching CRC `BB15A654` and simulator diff success. `ui_screen.c` owns one cleanup/fallback dispatch and passes the canonical content root explicitly to every top-level renderer; `ui_modal.c` hides the previous active sheet before showing another and preserves explicit nested parent returns. The remaining #16/#6 work is to own Messages, Nodes/routes, Map, Packets, and the Settings sheet domains; add UI-task command ownership; finish sheet-specific keyboard submit/cancel ownership; and deepen single-root/callback-lifetime invariants. | Pixel-neutral module slices use host tests plus default Actions. Modal/task/lifecycle changes require the smallest targeted COM12 interaction proof named by the slice. The final frozen release commit still needs commit-matched gate evidence. |
...
1. Continue #16/#6 UI modular ownership slices from the owned Home/Settings renderers, explicit-root contract, and exclusive modal manager toward a bounded UI command queue, Messages, Nodes/routes, Map, Packets, and Settings sheet domains.
