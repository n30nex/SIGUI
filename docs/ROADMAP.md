...
|---|---|---|
| UI modular ownership and current hierarchy proof | Navigation, shared chrome/layout, Home state/geometry/rendering, the top-level More hierarchy/actions, keyboard policy, the screen-root boundary, and single-active modal ownership are split across `ui_navigation.c`, `ui_chrome.c`, `ui_home.c`, `ui_settings.c`, `ui_keyboard.c`, `ui_screen.c`, and `ui_modal.c`. PR #56 proves the five-destination Home hierarchy; PR #57 proves dockless modals; PR #58 proves the full-height DM Thread. Merged PR #59 / source `d24552e` / Actions `29064260772` proves Contact Detail, Contact Options, and confirmation-only Forget on COM12 with exact CRC matches, passing simulator diffs, and a clean three-round all-tab probe; it did not invoke deletion, Public RF, or SD formatting. The next host slice makes Mesh Roles a full-height root menu with separate bounded read-only Rooms and Repeaters pages. The remaining #16/#6 work is to own the Messages renderer and Network/routes, Map, Packets, contact, and settings page domains; add UI-task command ownership; finish sheet-specific keyboard submit/cancel ownership; and deepen single-root/callback-lifetime invariants. | Build the Mesh Roles slice in Actions and capture `mesh_roles`, `mesh_rooms`, and `mesh_repeaters` on COM12, then physically verify Back and list scrolling. Continue the nested-page pattern through remaining Network and system surfaces. The final frozen release commit still needs commit-matched gate evidence. |
...
1. Prove the host-complete Mesh Roles root/Rooms/Repeaters hierarchy on exact Actions-built COM12 pixels, Back paths, and bounded scrolling, then continue the screenshot-audited #16/#6 slices through owned full-height Messages, Network/routes, Map, Packets, contact, and settings page domains before adding the bounded UI command queue.
