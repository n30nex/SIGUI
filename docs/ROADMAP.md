...
|---|---|---|
| UI modular ownership and current Home proof | Navigation, shared chrome/layout, Home state/geometry/rendering, the top-level More hierarchy/actions, keyboard policy, the screen-root boundary, and single-active modal ownership are split into `ui_navigation.c`, `ui_chrome.c`, `ui_home.c`, `ui_settings.c`, `ui_keyboard.c`, `ui_screen.c`, and `ui_modal.c`. PR #56 replaces the duplicate launcher/dock/action-grid structure with Home, Messages, Network, Map, and More; source `51258ba` / Actions `29060900359` passed COM12 Home capture with CRC `E72745BA` plus a clean all-tab probe. PR #57 makes all 26 modal/editing states dockless and passed an exact COM12 Public Search capture with `dock_hidden=true`. PR #58 / source `678bbcc` / Actions `29062818868` proves the full-height DM Thread on COM12 with CRC `1214617A`, passing simulator/scroll evidence, and a clean three-round live-data probe. The current host contact slice reduces Contact Detail to Message plus Contact Options, makes child pages return to Contact Options, and gates deletion behind a dedicated confirmation callback. The remaining #16/#6 work is to own the Messages renderer and Network/routes, Map, Packets, contact, and settings page domains; add UI-task command ownership; finish sheet-specific keyboard submit/cancel ownership; and deepen single-root/callback-lifetime invariants. | Build the contact hierarchy in Actions and capture its exact COM12 pixels/return paths without confirming deletion, then continue the nested-page pattern through remaining Network and system surfaces. The final frozen release commit still needs commit-matched gate evidence. |
...
1. Prove the host-complete contact hierarchy on exact Actions-built COM12 pixels and touch return paths, then continue the screenshot-audited #16/#6 slices through owned full-height Messages, Network/routes, Map, Packets, contact, and settings page domains before adding the bounded UI command queue.
