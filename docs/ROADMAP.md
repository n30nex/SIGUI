...
- Optional SD-card data storage. Status: important production feature with ESP32-side RP2040 SD query/setup plumbing, a CI-buildable RP2040 SD bridge target, generic bounded `DESKOS_SD_FILE v=1` file operations, and a retained blob-store abstraction. The first low-risk migration canary is now packet-log storage: when the RP2040 reports a ready card, file operations, matching line/path/chunk limits, and atomic rename, `packet_log_backend` can switch to `sd`, `data_backend` reports `mixed`, writes commit through `stores/packet_log/ring.tmp` to `stores/packet_log/ring.bin`, and NVS remains mirrored as fallback. With no card, timeout, missing file support, missing atomic rename, or insufficient limits, packet-log stays on NVS. `storage setup` and the Settings Storage Setup sheet remain non-destructive, and `DESKOS_SD_FORMAT FORMAT-DESKOS-SD` is still guarded behind exact confirmation plus bridge-reported present card, format support, and setup-required state. Later SD targets remain Public/DM message history, route history, diagnostic exports, and future map-tile caches. Settings, identity, and minimum boot-critical state should stay safe in onboard storage so the device remains usable without a card.
