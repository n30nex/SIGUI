...
|---|---|---|
| UI modular ownership and newest Home proof | Navigation ownership, the modal helper boundary, shared chrome/layout policy, compose-probe keyboard target/geometry policy, compose/non-compose keyboard layout and binding configuration, and the single-root screen render/content-root boundary are split into `ui_navigation.c`, `ui_modal.c`, `ui_chrome.c`, `ui_keyboard.c`, and `ui_screen.c`. PR #40 closed the current Home pixel proof on COM12 from `b2ca00a` / Actions `28731948363`. The remaining #16/#6 work is to finish owned screen/render modules, the remaining keyboard event/focus wiring, and deeper single-root invariants without broad UI cycling. | For code-only module slices, use host tests plus default Actions. For future one-surface Home/pixel acceptance, run only the matching COM12 `ui_capture_d1l.py` proof. |
...
| Physical screenshots/review | Host simulator screenshots are committed under `docs/screenshots`; PR #40 refreshes the Home simulator screenshot and captured COM12 Home pixels for the newest small-title/full-screen icon layout. | Add physical device photos and manual UI review artifact. |
...
- Icon Home proof: PR #40 host/simulator evidence, Actions run `28731948363`, and COM12 `ui_pixel_capture_b2ca00a_actions_28731948363_COM12.json` show Home renders a small-title full-screen icon launcher with colored Time/Wi-Fi/BLE/SD status icons and keeps the bottom dock off the Home screen while non-Home pages keep the dock. The capture reconstructed a 480x480 PNG with CRC `0DB2C27A`, simulator diff `ok=true`, `public_rf_tx=false`, and `formats_sd=false`.
