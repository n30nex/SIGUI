...
|---|---|---|
| UI modular ownership and newest Home proof | Navigation ownership, the modal helper boundary, shared chrome/layout policy, compose-probe keyboard target/geometry policy, compose keyboard layout/configuration, and the single-root screen render/content-root boundary are split into `ui_navigation.c`, `ui_modal.c`, `ui_chrome.c`, `ui_keyboard.c`, and `ui_screen.c`. PR #40 closed the current Home pixel proof on COM12 from `b2ca00a` / Actions `28731948363`. The remaining #16/#6 work is to finish owned screen/render modules, the remaining non-compose keyboard widget wiring, and deeper single-root invariants without broad UI cycling. | For code-only module slices, use host tests plus default Actions. For future one-surface Home/pixel acceptance, run only the matching COM12 `ui_capture_d1l.py` proof. |
