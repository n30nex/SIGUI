...
- Wi-Fi scan/connect screen. Status: Phase 5 status/persistence foundation added; `wifi status`, safe `wifi scan`, `wifi save <ssid> [password]`, `wifi clear`, `wifi on|off`, and Settings-tab Wi-Fi profile controls are implemented, but DeskOS does not start the live Wi-Fi runtime yet.
...
- Wi-Fi can be disabled and remains disabled across reboot if user chooses. Status: `wifi off` persists disabled state through the settings model, and schema v5 now also persists clearable Wi-Fi profile metadata for the future measured runtime path.
...
- Optional SD-card data storage. Status: important production feature with ESP32-side RP2040 SD query/setup plumbing, a CI-buildable RP2040 SD bridge target, generic bounded `DESKOS_SD_FILE v=1` file operations, and a retained blob-store abstraction. Retained Public message history, DM message history, route history, and packet history can now switch to SD when the RP2040 reports a ready card, file operations, matching line/path/chunk limits, and atomic rename. The stores commit through `stores/messages/public/public.tmp` -> `stores/messages/public/public.bin`, `stores/messages/dm/threads.tmp` -> `stores/messages/dm/threads.bin`, `stores/routes/routes.tmp` -> `stores/routes/routes.bin`, and `stores/packet_log/ring.tmp` -> `stores/packet_log/ring.bin`; NVS remains mirrored as fallback. With no card, timeout, missing file support, missing atomic rename, insufficient limits, or invalid SD blobs, retained history falls back to NVS. `storage setup` and the Settings Storage Setup sheet remain non-destructive, and `DESKOS_SD_FORMAT FORMAT-DESKOS-SD` is still guarded behind exact confirmation plus bridge-reported present card, format support, and setup-required state. Diagnostic SD exports are implemented through `storage export-canary <token>`, `storage export-diagnostics <token>`, `scripts/sd_export_canary_d1l.py`, and `scripts/sd_diagnostic_export_d1l.py`: they write under `exports/diagnostics/` by temp write/read plus atomic rename, leave final JSON present, support chunked diagnostic bundles, report map-tile cache readiness, and report no Public RF or formatting. Sampled user-data SD exports are implemented through `storage export-data <token>` and `scripts/sd_data_export_d1l.py`: they write under `exports/data/`, include recent messages, DMs, routes, packets, contacts, nodes, settings summary, manual map-center summary, and read-state, exclude private identity material, and report no Public RF or formatting. The first map-tile cache policy is implemented through `storage map-policy`, the touch Map tab, and host simulator coverage: it exposes the offline cache path `map/tiles/z{z}/x{x}/y{y}.tile`, cache readiness, disabled live-download state, manual center state, and no Public RF or formatting. The map-tile cache canary is implemented through `storage map-tile-canary <token>` and `scripts/sd_map_tile_canary_d1l.py`; it commits a synthetic tile under `map/tiles/` by temp write/read plus atomic rename and leaves the final tile present for inspection. The read-only `storage map-tile-check <token>` path and `scripts/sd_reboot_remount_acceptance_d1l.py` now provide the retained-store plus map-tile reboot/remount acceptance flow; real-card hardware proof remains pending. The later SD target remains live network tile download after Wi-Fi runtime and explicit user opt-in. Settings, identity, contacts, read-state, and minimum boot-critical state should stay safe in onboard storage so the device remains usable without a card.
- Optional manual location/map view. Status: first offline Map tab implemented with cache/download policy, retained route/node evidence, serial-configured manual center through `map center set <lat> <lon>`, a first-open touch location picker with pan/zoom/drop controls, persisted D1L pin state through the app model/settings schema v5 migration, sampled export summary, and simulator coverage. GPS/location-source integration, browser/geolocation integration, nearby GPS node pins, SD tile rendering/sideloading, and live network tile downloads remain pending; the view should use optional SD-card data for map tiles and offline map/cache data when a card is configured.
...
- SD-card data storage is entirely optional for end users. Status: ESP32-side safe `storage status`, explicit non-formatting `storage mount`, `storage setup`, `storage diag`, touch Storage Setup surfaces, host protocol simulator, a guarded confirmed-format path, GitHub Actions-built RP2040 bridge firmware, bounded `DESKOS_SD_FILE v=1` file operations, SD-capable retained Public/DM message history, route history, packet history, diagnostic exports, sampled user-data exports, a first offline Map tab/cache policy, and a map-tile cache canary are implemented with onboard fallback and no boot-time or incidental formatting.
- SD bridge hardware update 2026-07-01: Actions run `28551321662` for commit `8150b7b` produced verified ESP32 release and RP2040 SD bridge artifacts; the downloaded firmware, release package, and RP2040 checksum manifests pass, and `artifacts/hardware/com12/smoke_8150b7b.json` passed after flashing that package to COM12. The RP2040 UF2 checksum is `032FF80A0F94613BB18742E08CB97AA548BFF81BD627FF882C3AFACAF15F5C01`, but `artifacts/hardware/com12/rp2040_uf2_volumes_8150b7b_after_esp32_flash.json` found `candidate_volumes=[]`, so the bridge cannot be copied until the RP2040 is placed in UF2/BOOTSEL mode. `artifacts/hardware/com12/rp2040_preflight_8150b7b_after_format_timeout_settle.json` proves the RP2040 UART/ping/protocol/diag paths respond and the inserted card reaches `setup_required` with raw-card-present/mount-failed diagnostics. The guarded `artifacts/hardware/com12/sd_boot_prepare_unformatted_8150b7b.json` format attempt stayed safe but timed out before a confirmed format reply or ready file-operation gate; the current follow-up patch streams SdFat progress, keeps the ESP32 confirmed-format exchange alive only while progress arrives, and records `format_command_sent` separately from `format_confirmed` before the next Actions-built flash/retest.
