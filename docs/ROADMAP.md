...
- Optional SD-card data storage. Status: important production feature with ESP32-side RP2040 SD query/setup plumbing, a CI-buildable RP2040 SD bridge target, and a generic bounded `DESKOS_SD_FILE v=1` file-operation protocol implemented for future store work. The current D1L ESP32 BSP exposes no direct SDMMC/SDSPI card pins, so `storage status` uses a short `DESKOS_SD_STATUS` RP2040 UART probe, reports whether the bridge protocol/card/filesystem/root are ready, and keeps all retained stores on onboard NVS/serial fallback. `storage setup` and the Settings Storage Setup sheet expose the next safe action, and the ESP32 side has a guarded `DESKOS_SD_FORMAT FORMAT-DESKOS-SD` request path that is only reachable after exact user confirmation plus a bridge-reported present card, `format_supported=true`, and setup-required state. Current ESP32 hardware validation remains non-destructive and NVS-backed until the RP2040 bridge is flashed through a documented RP2040 procedure and larger retained stores are migrated to SD. The first low-risk migration canary should be packet-log storage behind a retained blob-store abstraction; later SD targets are Public/DM message history, route history, diagnostic exports, and future map-tile caches. Settings, identity, and minimum boot-critical state should stay safe in onboard storage so the device remains usable without a card.
