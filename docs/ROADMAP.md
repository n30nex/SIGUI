...
- SD-card data storage is entirely optional for end users. Status: ESP32-side `storage status`/`storage setup`, touch Storage Setup surfaces, host protocol simulator, a guarded confirmed-format request path, GitHub Actions-built RP2040 bridge firmware, bounded generic file-operation protocol support, SD-capable retained Public/DM message, route, and packet history, and diagnostic SD exports are implemented with onboard fallback and no boot-time or incidental formatting. Actions-built ESP32/RP2040 images from run `28478756887` were flashed on COM12/COM16 and validate the live RP2040 SD protocol plus no-card fallback, but the current hardware still reports `sd.state=no_card`/`present=false`; the guarded format confirmation is correctly refused with `ESP_ERR_NOT_FOUND` and no format performed. Full acceptance still requires the D1L to electrically detect a card, bad/unformatted cards to be detected without blocking normal operation, formatting to execute only after explicit user confirmation and bridge support, map-tile stores to land, and SD-backed stores to survive reboot and card remount when configured.
