...
|---|---|---|
| UI modular ownership and current Home proof | Navigation, shared chrome/layout, Home state/geometry/rendering, the top-level More hierarchy/actions, keyboard policy, the screen-root boundary, and single-active modal ownership are split into `ui_navigation.c`, `ui_chrome.c`, `ui_home.c`, `ui_settings.c`, `ui_keyboard.c`, `ui_screen.c`, and `ui_modal.c`. PR #56 replaces the duplicate launcher/dock/action-grid structure with Home, Messages, Network, Map, and More, consolidates status, and moves leaf tools into disclosure categories. Source `51258ba` / Actions `29060900359` passed COM12 Home capture with CRC `E72745BA` plus a three-round all-tab health probe with zero failures. `ui_screen.c` owns one cleanup/fallback dispatch and passes the canonical content root explicitly to every top-level renderer; `ui_modal.c` hides the previous active sheet before showing another, while the shell hides the dock for every active modal and restores it only after the last modal closes. The remaining #16/#6 work is to own Messages, Network/routes, Map, Packets, and settings sheet domains; add UI-task command ownership; finish sheet-specific keyboard submit/cancel ownership; and deepen single-root/callback-lifetime invariants. | Continue with full-height nested message/system pages and issue-sized Actions/COM12 proof. The final frozen release commit still needs commit-matched gate evidence. |
...
1. Continue the screenshot-audited #16/#6 UI slices from the proven Home/More hierarchy and dockless-modal invariant into full-height Messages, Network/routes, Map, Packets, and settings pages, then add the bounded UI command queue.
