...
- Optional SD-card data storage. Status: important production feature with ESP32-side RP2040 SD query/setup plumbing, a CI-buildable RP2040 SD bridge target, a generic bounded `DESKOS_SD_FILE v=1` file-operation protocol implemented for future store work, and an NVS-only retained blob-store abstraction now routing packet-log persistence through a backend boundary without changing behavior. The current D1L ESP32 BSP exposes no direct SDMMC/SDSPI card pins, so `storage status` uses a short `DESKOS_SD_STATUS` RP2040 UART probe, reports whether the bridge protocol/card/filesystem/root are ready, and keeps all retained stores on onboard NVS/serial fallback. `storage setup` and the Settings Storage Setup sheet expose the next safe action, and the ESP32 side has a guarded `DESKOS_SD_FORMAT FORMAT-DESKOS-SD` request path that is only reachable after exact user confirmation plus a bridge-reported present card, `format_supported=true`, and setup-required state. Current ESP32 hardware validation remains non-destructive and NVS-backed until the RP2040 bridge is flashed through a documented RP2040 procedure and the blob-store gains an SD provider. The first low-risk migration canary remains packet-log storage; later SD targets are Public/DM message history, route history, diagnostic exports, and future map-tile caches. Settings, identity, and minimum boot-critical state should stay safe in onboard storage so the device remains usable without a card.
...
- SD-card data storage is entirely optional for end users. Status: ESP32-side `storage status`/`storage setup`, touch Storage Setup surfaces, host protocol simulator, a guarded confirmed-format request path, GitHub Actions-built RP2040 bridge firmware, and bounded generic file-operation protocol support are implemented with onboard fallback and no boot-time or incidental formatting. Full acceptance still requires the RP2040 bridge firmware to be safely flashed and validated on hardware, boot to remain normal with no card, bad/unformatted cards to be detected without blocking normal operation, formatting to execute only after explicit user confirmation and bridge support, and SD-backed stores to survive reboot and card remount when configured.
