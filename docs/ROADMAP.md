...
- SD bridge hardware update 2026-07-01: Actions run `28499319258` for commit `a8268073ae290567e26f27491c8ffa167f6f8d57` flashed the ESP32 on COM12 and copied the RP2040 UF2 with SHA-256 `AA71CD32C9433F1D57B1C3F243ABB2A7535728E6A3905C6A424A1E77D5F3E57E`. COM12 preflight proved `rp2040 ping` works with `protocol_supported=true`, `sd_touched=false`, file limits, and `atomic_rename=true`, but `storage status` and `storage diag` still timed out when touching SD. The next change keeps `DESKOS_SD_STATUS` safe/cached and adds explicit `DESKOS_SD_MOUNT` / `storage mount`; the next hardware step is Actions build, flash/copy, COM12 preflight, and then canaries only if the mount reports a ready file gate.
