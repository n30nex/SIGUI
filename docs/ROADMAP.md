...
- SD bridge hardware update 2026-07-01: Actions run `28550779389` for commit `540319d` produced verified ESP32 release and RP2040 SD bridge artifacts; the downloaded firmware, release package, and RP2040 checksum manifests pass, and `artifacts/hardware/com12/smoke_540319d.json` passed after flashing that package to COM12. The RP2040 UF2 checksum is `032FF80A0F94613BB18742E08CB97AA548BFF81BD627FF882C3AFACAF15F5C01`, but `artifacts/hardware/com12/rp2040_uf2_volumes_540319d_after_esp32_flash.json` found `candidate_volumes=[]`, so the updated bridge has not been copied by UF2 mass storage. `artifacts/hardware/com12/rp2040_preflight_540319d_after_esp32_flash.json` proves the RP2040 UART/ping/protocol/diag paths respond and the inserted card reaches `setup_required` without Public RF or formatting. The guarded `artifacts/hardware/com12/sd_boot_prepare_unformatted_540319d.json` format attempt stayed safe but timed out before a ready file-operation gate, so the firmware and host runner now allow a longer confirmed-format window before the next Actions-built flash/retest.
...
- Final test report. Status: hardware validation notes are current through the 1-hour active Public soak, release package checkpoint, release-package sanity smoke, COM12 outbound DM proof against the COM11 Meshcorebot, the `540319d` current-commit COM12 smoke, and the current SD guarded-format timeout; `scripts/release_gate_audit_d1l.py` now produces a fail-closed release gate artifact with `ready_for_public_release=false` while P0 evidence is missing. Final full-release report is pending the SD matrix, 12-hour idle/listening soak, manual UI review/photos, full inbound/ACK/PATH/direct-route RF proof, and fresh COM12 smoke for whatever commit becomes final.
...
- Final docs match actual behavior. Status: docs are updated through the `68350bf` release-gate audit slice; final audit must pass with `ready_for_public_release=true` before a production tag.
