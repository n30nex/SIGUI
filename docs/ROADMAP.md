...
3. Validate and bank the current #69 retained-store source boundary before claiming SD fallback complete. Public and DM now use schema-v4 persisted clear lineages, prefer valid device-local NVS over a foreign removable lineage, roll back a merge if the SD generation changes, and keep independently dirty/retryable SD and NVS targets. Packet compact/journal persistence now reconciles before mutation, uses idempotent sequence-derived journal offsets, validates the real zero-byte-EOF bridge reply, and keeps independent SD/NVS/journal dirty state. The periodic worker and controlled reboot cover Public, DM, packet, and route stores. The first exact `3aefb9f` COM12 artifact passed broad smoke but was rejected after late-SD reconciliation reproduced a `route_persist` stack-overflow PANIC loop; the device was restored to checksum-verified `0399a316`. The 12 KiB worker/runner repair at `a2695fb` then passed exact Actions run `29183542983`, checksum-verified non-erasing COM12 flash, exact-commit smoke, and an SD file canary. The retained runner first exposed a legitimate planned one-time boot-help boundary bug; after that boundary was handled narrowly, a retained run exposed an intermittent post-ack `WDT` rather than the required `SW` reset. Preserve that crash evidence and keep PR #80 draft/fail-closed. The next-head repair candidate uses one shared 15-second reboot deadline, storage-manager quiesce, retained-worker force flush with the remaining time, final RP2040 bridge/UART quiesce with the remaining time, a final deadline check, and cancellation cleanup; successful restart keeps both quiesce locks held through the reset. Keep this item open until the new SHA passes the full host suite and Actions, its checksummed artifact is flashed to COM12, canonical SD file-canary/retained-history/reboot-remount artifacts pass, repeated immediate retained-canary reboot cycles all report `SW` with no new `WDT`/`PANIC`, the retained-worker margin remains healthy, and the later physical replacement-card matrix passes. Do not treat this source slice as all of #69.
...
- Retained repair commit `a2695fb` and Actions run `29183542983` have banked checksum-verified COM12 flash, exact-commit smoke, and SD file-canary proof. They are not final qualification: retained acceptance subsequently exposed the planned boot-help runner boundary and an intermittent post-ack `WDT`; the evidence remains preserved and PR #80 remains draft while the next-head quiesce repair awaits exact-artifact proof.
