...
- SD bridge hardware update 2026-07-01: Actions run `28500532119` for commit `cfb99620ee691097d845fb0e1920c06bbd02d1a5` flashed the ESP32 on COM12 and copied the RP2040 UF2 with SHA-256 `44F2D152818C3EDDEA60165B268234742D2EAD1281F345F7B059ADBB472E3B9C`. COM12 preflight `artifacts/hardware/com12/rp2040_sd_preflight_mount_split_28500532119.json` proved `rp2040 ping` works and safe `storage status` reports `state="mount_required"` without touching SD; the explicit `storage mount` path still timed out on the SD stack and left later SD commands unresponsive. The current follow-up moves SD mount/diag work to an RP2040 core1 worker and adds `state="mount_pending"` so UI polling and preflight remain responsive while a card probe is running; the next hardware step is Actions build, flash/copy, COM12 preflight, and then canaries only if the mount reports a ready file gate.
