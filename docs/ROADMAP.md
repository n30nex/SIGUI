...
|---|---|---|
| UI modular ownership and current Home proof | Navigation, shared chrome/layout, Home state/geometry/rendering, the top-level More hierarchy/actions, keyboard policy, the screen-root boundary, and single-active modal ownership are split into `ui_navigation.c`, `ui_chrome.c`, `ui_home.c`, `ui_settings.c`, `ui_keyboard.c`, `ui_screen.c`, and `ui_modal.c`. The screenshot-wide UX audit replaces the duplicate launcher/dock/action-grid structure with Home, Messages, Network, Map, and More; status is consolidated and leaf tools move into disclosure categories. PR #51 remains proof for the previous Home geometry only. `ui_screen.c` owns one cleanup/fallback dispatch and passes the canonical content root explicitly to every top-level renderer; `ui_modal.c` hides the previous active sheet before showing another and preserves explicit nested parent returns. The remaining #16/#6 work is to own Messages, Network/routes, Map, Packets, and settings sheet domains; add UI-task command ownership; finish sheet-specific keyboard submit/cancel ownership; and deepen single-root/callback-lifetime invariants. | Run host tests and all screenshot scenarios, then capture the hierarchy Home pixels plus a short navigation/corruption interaction on COM12 from the exact Actions artifact. The final frozen release commit still needs commit-matched gate evidence. |
...
| Physical screenshots/review | Host simulator screenshots are committed under `docs/screenshots`; PR #51 / Actions `29055441069` is historical proof for the prior Home geometry. | Capture the current hierarchy from the exact Actions artifact, then add physical device photos and the manual UI review artifact. |
...
1. Complete the screenshot-audited five-destination hierarchy, then continue #16/#6 UI modular ownership slices from the owned Home/More renderers, explicit-root contract, and exclusive modal manager toward a bounded UI command queue, Messages, Network/routes, Map, Packets, and settings sheet domains.
